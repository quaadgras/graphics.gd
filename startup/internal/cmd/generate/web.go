...
func generate_gdextension_web_cgo_callbacks() error {
	...
	fmt.Fprint(h, "EM_JS(void, go_on_init, (uint32_t level), {\n")
	fmt.Fprint(h, "\tGO.on_init(level);\n")
	fmt.Fprint(h, "});\n\n")
	...
	for fn := range api.StructureOf(&gdextension.On).Iter() {
		...
		// EM_JS(return_type, go_name, (params), { body });
		fmt.Fprintf(h, "EM_JS(%s, go_%s, (", retType, name)
...
func generate_startup_wasm_exports() error {
	...
	for fn := range api.StructureOf(&gdextension.On).Iter() {
		...
		// //go:wasmexport go_<name>
		fmt.Fprintf(f, "//go:wasmexport go_%s\nfunc go_%[1]s(", name)
		for i, arg := range args_flat(fn.Type, true) {
			if i > 0 {
				fmt.Fprint(f, ", ")
			}
