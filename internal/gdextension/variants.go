...
func (variant VariantType) Call(method StringName, args ...Variant) (Variant, error) {
	var err CallError
	var raw Variant
	Host.Builtin.Types.Call(variant, method, CallReturns[Variant](&raw), len(args), CallAccepts[Variant](unsafe.SliceData(args)), CallReturns[CallError](&err))
	return raw, err
}
...
func (variant VariantType) New(args ...Variant) (Variant, error) {
	var err CallError
	var raw Variant
	Host.Builtin.Types.Make(variant, CallReturns[Variant](&raw), len(args), CallAccepts[Variant](unsafe.SliceData(args)), CallReturns[CallError](&err))
	return raw, err
}
