...
func _init():
    ...
    var arr = self.array()
    if arr.size() != 3:                                  push_fail("Array.size()")
    if arr[0] != 1:                                  push_fail("Array[0]()"+str(arr[0]))
    if arr[1] != 2:                                  push_fail("Array[1]()")
    if arr[2] != 3:                                  push_fail("Array[2]()")
    if !valid_array(arr):                                push_fail("ValidArray()")

    var arrany = self.array_any()
    if arrany.size() != 3:                           push_fail("ArrayAny.size()")
    if arrany[0] != 1:                               push_fail("ArrayAny[0]")
    if arrany[1] != 2:                               push_fail("ArrayAny[1]")
    if arrany[2] != 3:                               push_fail("ArrayAny[2]")
    ...
    if !valid_node(self): push_fail("ValidNode()")
