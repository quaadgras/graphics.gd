...
func New() Instance {
	if !gd.Linked {
		...
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				...
				pointers.Set(gdclass.GetFBXDocument(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.FBXDocument{gdclass.NewFBXDocument(pointers.New[gd.Object]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))}))})
...
func init() {
	gdclass.Register("FBXDocument", func(ptr gd.Object) any { return Instance{gdclass.NewFBXDocument(ptr)} })
}
