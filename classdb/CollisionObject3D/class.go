...
/*
Abstract base class for 3D physics objects. [CollisionObject3D] can hold any number of [Shape3D]s for collision. Each shape must be assigned to a [i]shape owner[/i]. Shape owners are not nodes and do not appear in the editor, but are accessible through code using the shape_owner_* methods.
...
Note: [Instance.InputEvent] requires [member input_ray_pickable] to be true and at least one [member collision_layer] bit to be set.
*/
