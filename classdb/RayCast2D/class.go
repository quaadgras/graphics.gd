...
/*
A raycast represents a ray from its origin to its [Instance.TargetPosition] that finds the closest [graphics.gd/classdb/CollisionObject2D] along its path, if it intersects any.

[graphics.gd/classdb/RayCast2D] can ignore some objects by adding them to an exception list, by making its detection reporting ignore [graphics.gd/classdb/Area2D]s ([Instance.CollideWithAreas]) or [graphics.gd/classdb/PhysicsBody2D]s ([Instance.CollideWithBodies]), or by configuring physics layers.

[graphics.gd/classdb/RayCast2D] calculates intersection every physics frame, and it holds the result until the next physics frame. For an immediate raycast, or if you want to configure a [graphics.gd/classdb/RayCast2D] multiple times within the same physics frame, use [Instance.ForceRaycastUpdate].

To sweep over a region of 2D space, you can approximate the region with multiple [graphics.gd/classdb/RayCast2D]s or use [graphics.gd/classdb/ShapeCast2D].
*/
package RayCast2D
...
To get the intersected shape node, for a [graphics.gd/classdb/CollisionObject2D] target, use:
*/
...
/*
Returns the normal of the intersecting object's shape at the collision point, or Vector2(0, 0) if the ray starts inside the shape and [Instance.HitFromInside] is true.
...
/*
Adds a collision exception so the ray does not report collisions with the specified [graphics.gd/classdb/CollisionObject2D] node.
*/
...
/*
Removes a collision exception so the ray does report collisions with the specified [graphics.gd/classdb/CollisionObject2D] node.
*/
