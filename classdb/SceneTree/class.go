...
Note: The timer is always updated [i]after[/i] all of the nodes in the tree. A node's [Instance.Node.Process] method would be called before the timer updates (or [Instance.Node.PhysicsProcess] if 'process_in_physics' is set to true).
*/
...
Note: A [Tween] created using this method is not bound to any [Node]. It may keep working until there is nothing left to animate. If you want the [Tween] to be automatically killed when the [Node] is freed, use [Instance.Node.CreateTween] or [Instance.Tween.BindNode].
*/
...
/*
Queues the given 'obj' to be deleted, calling its [Instance.Object.Free] at the end of the current frame. This method is similar to [Instance.Node.QueueFree].
*/
...
/*
Calls [Instance.Object.Notification] with the given 'notification' to all nodes inside this tree added to the 'group'. Use 'call_flags' to customize this method's behavior (see [GroupCallFlags]).
*/
...
/*
Calls [Instance.Object.Notification] with the given 'notification' to all nodes inside this tree added to the 'group'. See also [url=$DOCS_URL/tutorials/best_practices/godot_notifications.html]Godot notifications[/url] and [Instance.CallGroup] and [Instance.SetGroup].
Note: This method acts immediately on all selected nodes at once, which may cause stuttering in some performance-intensive situations.
*/
...
/*
Returns the first [Node] found inside the tree, that has been added to the given 'group', in scene hierarchy order. Returns null if no match is found. See also [Instance.GetNodesInGroup].
*/
...
1. The current scene node is immediately removed from the tree. From that point, [Instance.Node.GetTree] called on the current (outgoing) scene will return null. [member current_scene] will be null, too, because the new scene is not available yet.
2. At the end of the frame, the formerly current scene, already removed from the tree, will be deleted (freed from memory) and then the new scene will be instantiated and added to the tree. [Instance.Node.GetTree] and [member current_scene] will be back to working as usual.
This ensures that both scenes aren't running at the same time, while still freeing the previous scene in a safe way similar to [Instance.Node.QueueFree].
*/
...
Returns [Ok] on success, [ErrUnconfigured] if no [member current_scene] is defined, [ErrCantOpen] if [member current_scene] cannot be loaded into a [PackedScene], or [ErrCantCreate] if the scene cannot be instantiated.
*/
...
/*
Searches for the [MultiplayerAPI] configured for the given path, if one does not exist it searches the parent paths until one is found. If the path is empty, or none is found, the default one is returned. See [Instance.SetMultiplayer].
*/
