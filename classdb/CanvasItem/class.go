...
Any [CanvasItem] can draw. For this, [Instance.QueueRedraw] is called by the engine, then [NotificationDraw] will be received on idle time to request a redraw. Because of this, canvas items don't need to be redrawn on every frame, improving the performance significantly. Several functions for drawing on the [CanvasItem] are provided (see draw_* functions). However, they can only be used inside [Instance.Draw], its corresponding [Instance.Object.Notification] or methods connected to the [signal draw] signal.
...
/*
Called when [CanvasItem] has been requested to redraw (after [Instance.QueueRedraw] is called, either manually or by the engine).
Corresponds to the [NotificationDraw] notification in [Instance.Object.Notification].
*/
...
/*
Returns true if the node is present in the [SceneTree], its [member visible] property is true and all its ancestors are also visible. If any ancestor is hidden, this node will not be visible in the scene tree, and is therefore not drawn (see [Instance.Draw]).
...
/*
Queues the [CanvasItem] to redraw. During idle time, if [CanvasItem] is visible, [NotificationDraw] is sent and [Instance.Draw] is called. This only occurs once per frame, even if this method has been called multiple times.
*/
...
/*
Draws a solid polygon of any number of points, convex or concave. Unlike [Instance.DrawColoredPolygon], each point's color can be changed individually. See also [Instance.DrawPolyline] and [Instance.DrawPolylineColors]. If you need more flexibility (such as being able to use bones), use [Instance.Renderingserver.CanvasItemAddTriangleArray] instead.
Note: If you frequently redraw the same polygon with a large number of vertices, consider pre-calculating the triangulation with [Instance.Geometry2d.TriangulatePolygon] and using [Instance.DrawMesh], [Instance.DrawMultimesh], or [Instance.Renderingserver.CanvasItemAddTriangleArray].
*/
...
See also [Instance.Font.DrawString].
*/
...
Equals to [Instance.GetGlobalTransform] if the window is embedded (see [member Viewport.gui_embed_subwindows]).
*/
...
Note: For screen-space coordinates (e.g. when using a non-embedded [Popup]), you can use [Instance.Displayserver.MouseGetPosition].
*/
...
/*
Set the value of a shader uniform for this instance only ([url=$DOCS_URL/tutorials/shaders/shader_reference/shading_language.html#per-instance-uniforms]per-instance uniform[/url]). See also [Instance.Shadermaterial.SetShaderParameter] to assign a uniform on all instances using the same [ShaderMaterial].
