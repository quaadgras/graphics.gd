...
Immediate collision overlaps can be done with the [member target_position] set to Vector3(0, 0, 0) and by calling [Instance.ForceShapecastUpdate] within the same physics frame. This helps to overcome some limitations of [Area3D] when used as an instantaneous detection area, as collision information isn't immediately available to it.
...
/*
Returns the collided [Object] of one of the multiple collisions at 'index', or null if no object is intersecting the shape (i.e. [Instance.IsColliding] returns false).
*/
...
func (self class) GetCollisionPoint(index int64) Vector3.XYZ { //gd:ShapeCast3D.get_collision_point
	var r_ret = gdextension.Call[Vector3.XYZ](gd.ObjectChecked(self.AsObject()), methods.get_collision_point, gdextension.SizeVector3|(gdextension.SizeInt<<4), &struct{ index int64 }{index})
	var ret = r_ret
	return ret
}
...
func (self class) GetCollisionNormal(index int64) Vector3.XYZ { //gd:ShapeCast3D.get_collision_normal
	var r_ret = gdextension.Call[Vector3.XYZ](gd.ObjectChecked(self.AsObject()), methods.get_collision_normal, gdextension.SizeVector3|(gdextension.SizeInt<<4), &struct{ index int64 }{index})
	var ret = r_ret
	return ret
}
