...
/*
Sets the tab size for the [graphics.gd/classdb/TextEdit] to use.
*/
...
/*
Returns the [graphics.gd/classdb/TextEdit]'s' tab size.
*/
...
/*
Performs a full reset of [graphics.gd/classdb/TextEdit], including undo history.
*/
...
/*
Returns the last unhidden line in the entire [graphics.gd/classdb/TextEdit].
*/
...
/*
Returns the count to the next visible line from 'line' to line + visible_amount. Can also count backwards. For example if a [graphics.gd/classdb/TextEdit] has 5 lines with lines 2 and 3 hidden, calling this with line = 1, visible_amount = 1 would return 3.
*/
...
/*
Called when the user presses the backspace key. Can be overridden with [Interface.Backspace].
*/
...
/*
Cut's the current selection. Can be overridden with [Interface.Cut].
*/
...
/*
Copies the current text selection. Can be overridden with [Interface.Copy].
*/
...
/*
Paste at the current location. Can be overridden with [Interface.Paste].
*/
...
An action will also end after a call to [Instance.EndAction], after [graphics.gd/classdb/ProjectSettings] "gui/timers/text_edit_idle_detect_sec" is triggered or a new undoable step outside the [Instance.StartAction] and [Instance.EndAction] calls.
*/
...
/*
Returns the current version of the [graphics.gd/classdb/TextEdit]. The version is a count of recorded operations by the undo/redo history.
*/
...
Note: The Y position corresponds to the bottom side of the line. Use [Instance.GetRectAtLineColumn] to get the top side position.
*/
...
Note: The Y position of the returned rect corresponds to the top side of the line, unlike [Instance.GetPosAtLineColumn] which returns the bottom side.
*/
...
/*
Returns the number of carets in this [graphics.gd/classdb/TextEdit].
*/
...
If 'include_ignored_carets' is false, carets from [Instance.MulticaretEditIgnoreCaret] will be ignored.
*/
...
If [Instance.IsInMulitcaretEdit] is true, carets that are collapsed will be true for [Instance.MulticaretEditIgnoreCaret].

[Instance.MergeOverlappingCarets] will be called if any carets were collapsed.
*/
...
If [Instance.IsInMulitcaretEdit] is true, the merge will be queued to happen at the end of the multicaret edit. See [Instance.BeginMulticaretEdit] and [Instance.EndMulticaretEdit].
...
Note: [Instance.IsCaretVisible] does not account for a caret being off-screen if it is still within the scrollable area. It will return true even if the caret is off-screen as long as it meets [graphics.gd/classdb/TextEdit]'s own conditions for being visible. This includes uses of [Instance.ScrollFitContentWidth] and [Instance.ScrollFitContentHeight] that cause the [graphics.gd/classdb/TextEdit] to expand beyond the viewport's bounds.
*/
...
/*
Moves the caret to the specified 'line' index. The caret column will be moved to the same visual position it was at the last time [Instance.SetCaretColumn] was called, or clamped to the end of the line.
...
If 'wrap_index' is -1, the caret column will be clamped to the 'line”s length. If 'wrap_index' is greater than -1, the column will be moved to attempt to match the visual x position on the line's 'wrap_index' to the position from the last time [Instance.SetCaretColumn] was called.

Note: If supporting multiple carets this will not check for any overlap. See [Instance.MergeOverlappingCarets].
*/
...
If [Instance.SelectingEnabled] is false, no selection will occur.
*/
...
If a selection's end column ([Instance.GetSelectionToColumn]) is at column 0, that line will not be included. If a selection begins on the line after another selection ends and 'merge_adjacent' is true, or they begin and end on the same line, one line range will include both selections.
*/
...
/*
Returns the [graphics.gd/classdb/VScrollBar] of the [graphics.gd/classdb/TextEdit].
*/
...
/*
Returns the [graphics.gd/classdb/HScrollBar] used by [graphics.gd/classdb/TextEdit].
*/
...
/*
Returns the total number of lines in the text. This includes wrapped lines and excludes folded lines. If [Instance.WrapMode] is set to [LineWrappingNone] and no lines are folded (see [graphics.gd/classdb/CodeEdit.Instance.IsLineFolded]) then this is equivalent to [Instance.GetLineCount]. See [Instance.GetVisibleLineCountInRange] for a limited range of lines.
*/
...
/*
Centers the viewport on the line the editing caret is at. This also resets the [Instance.ScrollHorizontal] value to 0.
*/
...
/*
Register a new gutter to this [graphics.gd/classdb/TextEdit]. Use 'at' to have a specific gutter order. A value of -1 appends the gutter to the right.
*/
...
/*
Set a custom draw callback for the gutter at the given index. 'draw_callback' must take the following arguments: A line index int, a gutter index int, and an area [Rect2.PositionSize]. This callback only works when the gutter type is [GutterTypeCustom] (see [Instance.SetGutterType]).
*/
...
/*
If 'clickable' is true, makes the 'gutter' on the given 'line' clickable. This is like [Instance.SetGutterClickable], but for a single line. If [Instance.IsGutterClickable] is true, this will not have any effect. See [Instance.IsLineGutterClickable] and [Instance.OnGutterClicked].
*/
...
/*
Returns the [graphics.gd/classdb/PopupMenu] of this [graphics.gd/classdb/TextEdit]. By default, this menu is displayed when right-clicking on the [graphics.gd/classdb/TextEdit].
...
const (
	...
	// Erases the whole [graphics.gd/classdb/TextEdit] text.
	MenuClear MenuItems = 3
	// Selects the whole [graphics.gd/classdb/TextEdit] text.
...
const (
	// When a gutter is set to string using [Instance.SetGutterType], it is used to contain text set via the [Instance.SetLineGutterText] method.
	GutterTypeString GutterType = 0
	// When a gutter is set to icon using [Instance.SetGutterType], it is used to contain an icon set via the [Instance.SetLineGutterIcon] method.
	GutterTypeIcon GutterType = 1
	// When a gutter is set to custom using [Instance.SetGutterType], it is used to contain custom visuals controlled by a callback method set via the [Instance.SetGutterCustomDraw] method.
