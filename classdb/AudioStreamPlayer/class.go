...
Note: The position is not always accurate, as the [AudioServer] does not mix audio every processed frame. To get more accurate results, add [Instance.Audioserver.GetTimeSinceLastMix] to the returned position.
...
/*
Returns true if any sound is active, even if [member stream_paused] is set to true. See also [member playing] and [Instance.GetStreamPlayback].
*/
...
/*
Returns the latest [AudioStreamPlayback] of this node, usually the most recently created by [Instance.Play]. If no sounds are playing, this method fails and returns an empty playback.
*/
...
func (self class) SetMixTarget(mix_target MixTarget) { //gd:AudioStreamPlayer.set_mix_target
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.set_mix_target, 0|(gdextension.SizeInt<<4), &struct{ mix_target MixTarget }{mix_target})
}

//go:nosplit
func (self class) GetMixTarget() MixTarget { //gd:AudioStreamPlayer.get_mix_target
	var r_ret = gdextension.Call[MixTarget](gd.ObjectChecked(self.AsObject()), methods.get_mix_target, gdextension.SizeInt, &struct{}{})
	var ret = r_ret
	return ret
}
