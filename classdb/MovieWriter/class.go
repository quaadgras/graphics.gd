...
If you need to encode to a different format or pipe a stream through third-party software, you can extend the [graphics.gd/classdb/MovieWriter] class to create your own movie writers. This should typically be done using GDExtension for performance reasons.
...
type Interface interface {
	// Called when the audio sample rate used for recording the audio is requested by the engine. The value returned must be specified in Hz. Defaults to 48000 Hz if [Interface.GetAudioMixRate] is not overridden.
	GetAudioMixRate() int
	// Called when the audio speaker mode used for recording the audio is requested by the engine. This can affect the number of output channels in the resulting audio file/stream. Defaults to [Audioserver.SpeakerModeStereo] if [Interface.GetAudioSpeakerMode] is not overridden.
	...
	// Called when the engine finishes writing. This occurs when the engine quits by pressing the window manager's close button, or when [graphics.gd/classdb/SceneTree.Instance.Quit] is called.
	//
	// Note: Pressing Ctrl + C on the terminal running the editor/project does not result in [Interface.WriteEnd] being called.
	WriteEnd()
}
...
/*
Adds a writer to be usable by the engine. The supported file extensions can be set by overriding [Interface.HandlesFile].
