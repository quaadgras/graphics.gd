...
[Expression] objects can be used to define conversions between the data, such as when glTF defines an angle in radians and Godot uses degrees. The [member object_model_type] property defines the type of data stored in the glTF file as defined by the object model, see [GLTFObjectModelType] for possible values.
*/
...
/*
High-level wrapper over [Instance.AppendNodePath] that handles the most common cases. It constructs a new [NodePath] using 'node_path' as a base and appends 'prop_name' to the subpath. Be sure to also call [Instance.SetTypes] once (the order does not matter).
*/
...
/*
The GLTF accessor type associated with this property's [member object_model_type]. See [member GLTFAccessor.accessor_type] for possible values, and see [GLTFObjectModelType] for how the object model type maps to accessor types.
*/
