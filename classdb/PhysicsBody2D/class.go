...
/*
Moves the body along the vector 'motion'. In order to be frame rate independent in [Instance.Node.PhysicsProcess] or [Instance.Node.Process], 'motion' should be computed using delta.
...
/*
Checks for collisions without moving the body. In order to be frame rate independent in [Instance.Node.PhysicsProcess] or [Instance.Node.Process], 'motion' should be computed using delta.
...
'safe_margin' is the extra margin used for collision recovery (see [member CharacterBody2D.safe_margin] for more details).
If 'recovery_as_collision' is true, any depenetration from the recovery phase is also reported as a collision; this is useful for checking whether the body would [i]touch[/i] any other bodies.
*/
