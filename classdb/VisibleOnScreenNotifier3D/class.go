...
/*
[graphics.gd/classdb/VisibleOnScreenNotifier3D] represents a box-shaped region of 3D space. When any part of this region becomes visible on screen or in a [graphics.gd/classdb/Camera3D]'s view, it will emit a [Instance.OnScreenEntered] signal, and likewise it will emit a [Instance.OnScreenExited] signal when no part of it remains visible.

If you want a node to be enabled automatically when this region is visible on screen, use [graphics.gd/classdb/VisibleOnScreenEnabler3D].

Note: [graphics.gd/classdb/VisibleOnScreenNotifier3D] uses an approximate heuristic that doesn't take walls and other occlusion into account, unless occlusion culling is used. It also won't function unless [graphics.gd/classdb/Node3D.Instance.Visible] is set to true.
*/
...
Note: It takes one frame for the [graphics.gd/classdb/VisibleOnScreenNotifier3D]'s visibility to be assessed once added to the scene tree, so this method will always return false right after it is instantiated.
*/
