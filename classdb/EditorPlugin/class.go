...
/*
Called when there is a root node in the current edited scene, [Instance.Handles] is implemented, and an [InputEvent] happens in the 2D viewport. If this method returns true, 'event' is intercepted by this [EditorPlugin], otherwise 'event' is forwarded to other Editor classes.
...
/*
Called by the engine when the 2D editor's viewport is updated. Use the overlay [Control] for drawing. You can update the viewport manually by calling [Instance.UpdateOverlays].
...
/*
This method is the same as [Instance.ForwardCanvasDrawOverViewport], except it draws on top of everything. Useful when you need an extra layer that shows over anything else.
...
/*
Called when there is a root node in the current edited scene, [Instance.Handles] is implemented, and an [InputEvent] happens in the 3D viewport. The return value decides whether the [InputEvent] is consumed or forwarded to other [EditorPlugin]s. See [AfterGUIInput] for options.
...
[/csharp]
*/
func (Instance) _forward_3d_gui_input(impl func(ptr gdclass.Receiver, viewport_camera Camera3D.Instance, event InputEvent.Instance) int) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args, p_back gdextension.Pointer) {
...
/*
Called by the engine when the 3D editor's viewport is updated. Use the overlay [Control] for drawing. You can update the viewport manually by calling [Instance.UpdateOverlays].
...
/*
This method is the same as [Instance.Forward3dDrawOverViewport], except it draws on top of everything. Useful when you need an extra layer that shows over anything else.
You need to enable calling of this method by using [Instance.SetForceDrawOverForwardingEnabled].
*/
...
When the plugin's workspace is selected, other main screen plugins will be hidden, but your plugin will not appear automatically. It needs to be added as a child of [Instance.Editorinterface.GetEditorMainScreen] and made visible inside [Instance.MakeVisible].
Use [Instance.GetPluginName] and [Instance.GetPluginIcon] to customize the plugin button's appearance.
[codeblock]
var plugin_control
...
/*
Implement this function if your plugin edits a specific type of object (Resource or Node). If you return true, then you will get the functions [Instance.Edit] and [Instance.MakeVisible] called when the editor requests them. If you have declared the methods [Instance.ForwardCanvasGuiInput] and [Instance.Forward3dGuiInput] these will be called too.
...
/*
Override this method to provide a state data you want to be saved, like view position, grid settings, folding, etc. This is used when saving the scene (so state is kept when opening it again) and for switching tabs (so state can be restored when the tab returns). This data is automatically saved for each scene in an editstate file in the editor metadata folder. If you want to store global (scene-independent) editor data for your plugin, you can use [Instance.GetWindowLayout] instead.
Use [Instance.SetState] to restore your saved state.
Note: This method should not be used to save important settings that should persist with the project.
Note: You must implement [Instance.GetPluginName] for the state to be stored and restored correctly.
[codeblock]
func _get_state():
...
/*
Restore the state saved by [Instance.GetState]. This method is called when the current scene tab is changed in the editor.
Note: Your plugin must implement [Instance.GetPluginName], otherwise it will not be recognized and this method will not be called.
[codeblock]
func _set_state(data):
...
If the user confirms saving, [Instance.SaveExternalData] will be called, before closing the editor.
[codeblock]
func _get_unsaved_status(for_scene):
...
/*
Restore the plugin GUI layout and data saved by [Instance.GetWindowLayout]. This method is called for every plugin on editor startup. Use the provided 'configuration' file to read your saved data.
[codeblock]
func _set_window_layout(configuration):
...
Use [Instance.SetWindowLayout] to restore your saved layout.
[codeblock]
func _get_window_layout(configuration):
...
/*
This method is called when the editor is about to run the project. The plugin can then perform required operations before the project runs.
This method must return a boolean. If this method returns false, the project will not run. The run is aborted immediately, so this also prevents all other plugins' [Instance.Build] methods from running.
*/
...
When your plugin is deactivated, make sure to remove your custom control with [Instance.RemoveControlFromContainer] and free it with [Instance.Node.QueueFree].
*/
...
When your plugin is deactivated, make sure to remove your custom control with [Instance.RemoveControlFromDocks] and free it with [Instance.Node.QueueFree].
...
/*
Removes the control from the dock. You have to manually [Instance.Node.QueueFree] the control.
*/
...
/*
Removes the control from the bottom panel. You have to manually [Instance.Node.QueueFree] the control.
*/
...
/*
Removes the control from the specified container. You have to manually [Instance.Node.QueueFree] the control.
*/
...
/*
Adds a custom [PopupMenu] submenu under Project > Tools > 'name'. Use [Instance.RemoveToolMenuItem] on plugin clean up to remove the menu.
*/
...
You can use the virtual method [Instance.Handles] to check if your custom object is being edited by checking the script or using the is keyword.
...
/*
Updates the overlays of the 2D and 3D editor viewport. Causes methods [Instance.ForwardCanvasDrawOverViewport], [Instance.ForwardCanvasForceDrawOverViewport], [Instance.Forward3dDrawOverViewport] and [Instance.Forward3dForceDrawOverViewport] to be called.
*/
...
/*
Remove the [EditorScenePostImportPlugin], added with [Instance.AddScenePostImportPlugin].
*/
...
Note: Always use [Instance.RemoveInspectorPlugin] to remove the registered [EditorInspectorPlugin] when your [EditorPlugin] is disabled to prevent leaks and an unexpected behavior.
...
/*
Use this method if you always want to receive inputs from 3D view screen inside [Instance.Forward3dGuiInput]. It might be especially usable if your plugin will want to use raycast in the scene.
*/
...
/*
Enables calling of [Instance.ForwardCanvasForceDrawOverViewport] for the 2D editor and [Instance.Forward3dForceDrawOverViewport] for the 3D editor when their viewports are updated. You need to call this method only once and it will work permanently for this plugin.
*/
