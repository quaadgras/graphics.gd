...
/*
Godot loads resources in the editor or in exported games using ResourceFormatLoaders. They are queried automatically via the [graphics.gd/classdb/ResourceLoader] singleton, or when a resource with internal dependencies is loaded. Each file type may load as a different resource type, so multiple ResourceFormatLoaders are registered in the engine.

Extending this class allows you to define your own loader. Be sure to respect the documented return types and values. You should give it a global class name with class_name for it to be registered. Like built-in ResourceFormatLoaders, it will be called automatically when loading resources of its handled type(s). You may also implement a [graphics.gd/classdb/ResourceFormatSaver].

Note: You can also extend [graphics.gd/classdb/EditorImportPlugin] if the resource type you need exists but Godot is unable to load its format. Choosing one way over another depends on if the format is suitable or not for the final exported game. For example, it's better to import .png textures as .ctex ([graphics.gd/classdb/CompressedTexture2D]) first, so they can be loaded with better efficiency on the graphics card.
*/
...
type Interface interface {
	...
	//
	// If it is not implemented, the default behavior returns whether the path's extension is within the ones provided by [Interface.GetRecognizedExtensions], and if the type is within the ones provided by [Interface.GetResourceType].
	RecognizePath(path string, atype string) bool
	// Tells which resource class this loader can load.
	//
	// Note: Custom resource types defined by scripts aren't known by the [graphics.gd/classdb/ClassDB], so you might just handle "Resource" for them.
	...
	// Returns the script class name associated with the [graphics.gd/classdb/Resource] under the given 'path'. If the resource has no script or the script isn't a named class, it should return "".
	...
	// Loads a resource when the engine finds this loader to be compatible. If the loaded resource is the result of an import, 'original_path' will target the source file. Returns a [graphics.gd/classdb/Resource] object on success, or an [Error] constant in case of failure.
	//
