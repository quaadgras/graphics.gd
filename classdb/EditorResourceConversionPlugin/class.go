...
/*
[graphics.gd/classdb/EditorResourceConversionPlugin] is invoked when the context menu is brought up for a resource in the editor inspector. Relevant conversion plugins will appear as menu options to convert the given resource to a target type.

Below shows an example of a basic plugin that will convert an [graphics.gd/classdb/ImageTexture] to a [graphics.gd/classdb/PortableCompressedTexture2D].
...
To use an [graphics.gd/classdb/EditorResourceConversionPlugin], register it using the [graphics.gd/classdb/EditorPlugin.Instance.AddResourceConversionPlugin] method first.
*/
...
type Interface interface {
	// Returns the class name of the target type of [graphics.gd/classdb/Resource] that this plugin converts source resources to.
	ConvertsTo() string
	// Called to determine whether a particular [graphics.gd/classdb/Resource] can be converted to the target resource type by this plugin.
	Handles(resource Resource.Instance) bool
	// Takes an input [graphics.gd/classdb/Resource] and converts it to the type given in [Interface.ConvertsTo]. The returned [graphics.gd/classdb/Resource] is the result of the conversion, and the input [graphics.gd/classdb/Resource] remains unchanged.
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.EditorResourceConversionPlugin](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.EditorResourceConversionPlugin{pointers.Add[gdclass.EditorResourceConversionPlugin]([3]uint64{})})
	...
	casted := Instance([1]gdclass.EditorResourceConversionPlugin{pointers.New[gdclass.EditorResourceConversionPlugin]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func (self class) AsEditorResourceConversionPlugin() Advanced {
	return Advanced{pointers.AsA[gdclass.EditorResourceConversionPlugin](self[0])}
}
func (self Instance) AsEditorResourceConversionPlugin() Instance {
	return Instance{pointers.AsA[gdclass.EditorResourceConversionPlugin](self[0])}
}
...
func init() {
	gdclass.Register("EditorResourceConversionPlugin", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.EditorResourceConversionPlugin](ptr)} })
}
