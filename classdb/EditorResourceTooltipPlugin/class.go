...
A plugin must be first registered with [Instance.Filesystemdock.AddResourceTooltipPlugin]. When the user hovers a resource in filesystem dock which is handled by the plugin, [Instance.MakeTooltipForPath] is called to create the tooltip. It works similarly to [Instance.Control.MakeCustomTooltip].
*/
...
The 'metadata' dictionary is provided by preview generator (see [Instance.Editorresourcepreviewgenerator.Generate]).
...
Note: It's unadvised to use [Instance.Resourceloader.Load], especially with heavy resources like models or textures, because it will make the editor unresponsive when creating the tooltip. You can use [Instance.RequestThumbnail] if you want to display a preview in your tooltip.
Note: If you decide to discard the 'base', make sure to call [Instance.Node.QueueFree], because it's not freed automatically.
[codeblock]
func _make_tooltip_for_path(path, metadata, base):
