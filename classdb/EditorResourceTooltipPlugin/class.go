...
/*
Resource tooltip plugins are used by [graphics.gd/classdb/FileSystemDock] to generate customized tooltips for specific resources. E.g. tooltip for a [graphics.gd/classdb/Texture2D] displays a bigger preview and the texture's dimensions.

A plugin must be first registered with [graphics.gd/classdb/FileSystemDock.Instance.AddResourceTooltipPlugin]. When the user hovers a resource in filesystem dock which is handled by the plugin, [Interface.MakeTooltipForPath] is called to create the tooltip. It works similarly to [graphics.gd/classdb/Control.Instance.MakeCustomTooltip].
*/
...
type Interface interface {
	// Return true if the plugin is going to handle the given [graphics.gd/classdb/Resource] 'type'.
	...
	//
	// The 'metadata' dictionary is provided by preview generator (see [graphics.gd/classdb/EditorResourcePreviewGenerator.Instance.Generate]).
	//
	// 'base' is the base default tooltip, which is a [graphics.gd/classdb/VBoxContainer] with a file name, type and size labels. If another plugin handled the same file type, 'base' will be output from the previous plugin. For best result, make sure the base tooltip is part of the returned [graphics.gd/classdb/Control].
	//
	// Note: It's unadvised to use [graphics.gd/classdb/ResourceLoader.Load], especially with heavy resources like models or textures, because it will make the editor unresponsive when creating the tooltip. You can use [Instance.RequestThumbnail] if you want to display a preview in your tooltip.
	//
	// Note: If you decide to discard the 'base', make sure to call [graphics.gd/classdb/Node.Instance.QueueFree], because it's not freed automatically.
	//
	//
	//
...
/*
Requests a thumbnail for the given [graphics.gd/classdb/TextureRect]. The thumbnail is created asynchronously by [graphics.gd/classdb/EditorResourcePreview] and automatically set when available.
*/
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.EditorResourceTooltipPlugin](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.EditorResourceTooltipPlugin{pointers.Add[gdclass.EditorResourceTooltipPlugin]([3]uint64{})})
	...
	casted := Instance([1]gdclass.EditorResourceTooltipPlugin{pointers.New[gdclass.EditorResourceTooltipPlugin]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func (self Instance) AsEditorResourceTooltipPlugin() Instance {
	return Instance{pointers.AsA[gdclass.EditorResourceTooltipPlugin](self[0])}
}
...
func init() {
	gdclass.Register("EditorResourceTooltipPlugin", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.EditorResourceTooltipPlugin](ptr)} })
}
