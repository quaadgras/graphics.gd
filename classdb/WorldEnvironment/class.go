...
The parameters defined in the [graphics.gd/classdb/WorldEnvironment] can be overridden by an [graphics.gd/classdb/Environment] node set on the current [graphics.gd/classdb/Camera3D]. Additionally, only one [graphics.gd/classdb/WorldEnvironment] may be instantiated in a given scene at a time.

The [graphics.gd/classdb/WorldEnvironment] allows the user to specify default lighting parameters (e.g. ambient lighting), various post-processing effects (e.g. SSAO, DOF, Tonemapping), and how to draw the background (e.g. solid color, skybox). Usually, these are added in order to improve the realism/color balance of the scene.
*/
