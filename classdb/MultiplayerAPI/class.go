...
const (
	// Used with [graphics.gd/classdb/Node.Instance.RpcConfig] to disable a method or property for all RPC calls, making it unavailable. Default for all methods.
	RpcModeDisabled RPCMode = 0
	// Used with [graphics.gd/classdb/Node.Instance.RpcConfig] to set a method to be callable remotely by any peer. Analogous to the @rpc("any_peer") annotation. Calls are accepted from all remote peers, no matter if they are node's authority or not.
	RpcModeAnyPeer RPCMode = 1
	// Used with [graphics.gd/classdb/Node.Instance.RpcConfig] to set a method to be callable remotely only by the current multiplayer authority (which is the server by default). Analogous to the @rpc("authority") annotation. See [graphics.gd/classdb/Node.Instance.SetMultiplayerAuthority].
