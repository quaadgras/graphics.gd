...
To get a [ViewportTexture] in code, use the [Instance.Viewport.GetTexture] method on the target viewport.
...
Note: When using a [Viewport] with [member Viewport.use_hdr_2d] set to true, the returned texture will be an HDR image encoded in linear space. This may look darker than normal when displayed directly on screen. To convert to gamma space, you can do the following:
[codeblock]
img.convert(Image.FORMAT_RGBA8)
imb.linear_to_srgb()
[/codeblock]
Note: Some nodes such as [Decal], [Light3D], and [PointLight2D] do not support using [ViewportTexture] directly. To use texture data from a [ViewportTexture] in these nodes, you need to create an [ImageTexture] by calling [Instance.Texture2d.GetImage] on the [ViewportTexture] and passing the result to [Instance.Imagetexture.CreateFromImage]. This conversion is a slow operation, so it should not be performed every frame.
*/
