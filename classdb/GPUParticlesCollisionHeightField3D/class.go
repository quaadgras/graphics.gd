...
Heightmap shapes allow for efficiently representing collisions for convex and concave objects with a single "floor" (such as terrain). This is less flexible than [graphics.gd/classdb/GPUParticlesCollisionSDF3D], but it doesn't require a baking step.

[graphics.gd/classdb/GPUParticlesCollisionHeightField3D] can also be regenerated in real-time when it is moved, when the camera moves, or even continuously. This makes [graphics.gd/classdb/GPUParticlesCollisionHeightField3D] a good choice for weather effects such as rain and snow and games with highly dynamic geometry. However, this class is limited since heightmaps cannot represent overhangs (e.g. indoors or caves).

Note: [graphics.gd/classdb/ParticleProcessMaterial.Instance.CollisionMode] must be true on the [graphics.gd/classdb/GPUParticles3D]'s process material for collision to work.
...
/*
Based on 'value', enables or disables the specified layer in the [Instance.HeightfieldMask], given a 'layer_number' between 1 and 20, inclusive.
*/
...
/*
Returns true if the specified layer of the [Instance.HeightfieldMask] is enabled, given a 'layer_number' between 1 and 20, inclusive.
*/
...
const (
	// Only update the heightmap when the [graphics.gd/classdb/GPUParticlesCollisionHeightField3D] node is moved, or when the camera moves if [Instance.FollowCameraEnabled] is true. An update can be forced by slightly moving the [graphics.gd/classdb/GPUParticlesCollisionHeightField3D] in any direction, or by calling [graphics.gd/classdb/RenderingServer.ParticlesCollisionHeightFieldUpdate].
