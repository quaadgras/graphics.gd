...
Note: Keep in mind that [Instance.Node.GetChildren] will also return the connection layer node named _connection_layer due to technical limitations. This behavior may change in future releases.
*/
package GraphEdit
...
By default, a hot zone is a [Rect2] positioned such that its center is at 'in_node'.[Instance.Graphnode.GetInputPortPosition]('in_port') (For output's case, call [Instance.Graphnode.GetOutputPortPosition] instead). The hot zone's width is twice the Theme Property port_grab_distance_horizontal, and its height is twice the port_grab_distance_vertical.
Below is a sample code to help get started:
[codeblock]
func _is_in_input_hotzone(in_node, in_port, mouse_position):
...
/*
Returns whether the 'mouse_position' is in the output hot zone. For more information on hot zones, see [Instance.IsInInputHotzone].
Below is a sample code to help get started:
[codeblock]
func _is_in_output_hotzone(in_node, in_port, mouse_position):
...
/*
Allows to disconnect nodes when dragging from the right port of the [GraphNode]'s slot if it has the specified type. See also [Instance.RemoveValidRightDisconnectType].
*/
...
/*
Disallows to disconnect nodes when dragging from the right port of the [GraphNode]'s slot if it has the specified type. Use this to disable disconnection previously allowed with [Instance.AddValidRightDisconnectType].
*/
...
/*
Allows to disconnect nodes when dragging from the left port of the [GraphNode]'s slot if it has the specified type. See also [Instance.RemoveValidLeftDisconnectType].
*/
...
/*
Disallows to disconnect nodes when dragging from the left port of the [GraphNode]'s slot if it has the specified type. Use this to disable disconnection previously allowed with [Instance.AddValidLeftDisconnectType].
*/
...
/*
Allows the connection between two different port types. The port type is defined individually for the left and the right port of each slot with the [Instance.Graphnode.SetSlot] method.
...
/*
Disallows the connection between two different port types previously allowed by [Instance.AddValidConnectionType]. The port type is defined individually for the left and the right port of each slot with the [Instance.Graphnode.SetSlot] method.
...
/*
Returns whether it's possible to make a connection between two different port types. The port type is defined individually for the left and the right port of each slot with the [Instance.Graphnode.SetSlot] method.
