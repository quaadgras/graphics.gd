...
To use [EditorImportPlugin], register it using the [Instance.Editorplugin.AddImportPlugin] method first.
*/
...
/*
Gets the number of initial presets defined by the plugin. Use [Instance.GetImportOptions] to get the default options for the preset and [Instance.GetPresetName] to get the name of the preset.
*/
...
*/
func (Instance) _get_import_options(impl func(ptr gdclass.Receiver, path string, preset_index int) []map[any]any) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args, p_back gdextension.Pointer) {
...
*/
func (Instance) _get_option_visibility(impl func(ptr gdclass.Receiver, path string, option_name string, options map[any]any) bool) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args, p_back gdextension.Pointer) {
		var path = String.Via(gd.StringProxy{}, pointers.Pack(pointers.New[gd.String](gd.UnsafeGet[gdextension.String](p_args, 0))))
		defer pointers.End(gd.InternalString(path))
...
This method must be overridden to do the actual importing work. See this class' description for an example of overriding this method.
*/
func (Instance) _import(impl func(ptr gdclass.Receiver, source_file string, save_path string, options map[any]any, platform_variants []string, gen_files []string) error) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args, p_back gdextension.Pointer) {
...
/*
This function can only be called during the [Instance.Import] callback and it allows manually importing resources from it. This is useful when the imported file generates external resources that require importing (as example, images). Custom parameters for the ".import" file can be passed via the 'custom_options'. Additionally, in cases where multiple importers can handle a file, the 'custom_importer' can be specified to force a specific one. This function performs a resource import and returns immediately with a success or error code. 'generator_parameters' defines optional extra metadata which will be stored as generator_parameters in the remap section of the .import file, for example to store a md5 hash of the source data.
*/
