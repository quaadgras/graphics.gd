...
You can add each [WebRTCPeerConnection] via [Instance.AddPeer] or remove them via [Instance.RemovePeer]. Peers must be added in [Webrtcpeerconnection.StateNew] state to allow it to create the appropriate channels. This class will not create offers nor set descriptions, it will only poll them, and notify connections and disconnections.
When creating the peer via [Instance.CreateClient] or [Instance.CreateServer] the [Instance.Multiplayerpeer.IsServerRelaySupported] method will return true enabling peer exchange and packet relaying when supported by the [MultiplayerAPI] implementation.
...
/*
Initialize the multiplayer peer as a server (with unique ID of 1). This mode enables [Instance.Multiplayerpeer.IsServerRelaySupported], allowing the upper [MultiplayerAPI] layer to perform peer exchange and packet relaying.
...
Three channels will be created for reliable, unreliable, and ordered transport. The value of 'unreliable_lifetime' will be passed to the "maxPacketLifetime" option when creating unreliable and ordered channels (see [Instance.Webrtcpeerconnection.CreateDataChannel]).
*/
...
/*
Remove the peer with given 'peer_id' from the mesh. If the peer was connected, and [signal MultiplayerPeer.peer_connected] was emitted for it, then [signal MultiplayerPeer.peer_disconnected] will be emitted.
*/
