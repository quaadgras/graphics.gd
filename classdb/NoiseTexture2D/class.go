...
The class uses [Thread]s to generate the texture data internally, so [Instance.Texture2d.GetImage] may return null if the generation process has not completed yet. In that case, you need to wait for the texture to be generated before accessing the image and the generated byte data:
[codeblock]
