...
/*
Uses the [graphics.gd/classdb/FastNoiseLite] library or other noise generators to fill the texture data of your desired size. [graphics.gd/classdb/NoiseTexture2D] can also generate normal map textures.

The class uses [graphics.gd/classdb/Thread]s to generate the texture data internally, so [graphics.gd/classdb/Texture2D.Instance.GetImage] may return null if the generation process has not completed yet. In that case, you need to wait for the texture to be generated before accessing the image and the generated byte data:
...
func (self class) SetIn3dSpace(enable bool) { //gd:NoiseTexture2D.set_in_3d_space
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.set_in_3d_space, 0|(gdextension.SizeBool<<4), &struct{ enable bool }{enable})
}

//go:nosplit
func (self class) IsIn3dSpace() bool { //gd:NoiseTexture2D.is_in_3d_space
	var r_ret = gdextension.Call[bool](gd.ObjectChecked(self.AsObject()), methods.is_in_3d_space, gdextension.SizeBool, &struct{}{})
	var ret = r_ret
	return ret
}
