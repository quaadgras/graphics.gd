...
/*
[graphics.gd/classdb/EditorNode3DGizmoPlugin] allows you to define a new type of Gizmo. There are two main ways to do so: extending [graphics.gd/classdb/EditorNode3DGizmoPlugin] for the simpler gizmos, or creating a new [graphics.gd/classdb/EditorNode3DGizmo] type. See the tutorial in the documentation for more info.

To use [graphics.gd/classdb/EditorNode3DGizmoPlugin], register it using the [graphics.gd/classdb/EditorPlugin.Instance.AddNode3dGizmoPlugin] method first.
*/
...
type Interface interface {
	// Override this method to define which Node3D nodes have a gizmo from this plugin. Whenever a [graphics.gd/classdb/Node3D] node is added to a scene this method is called, if it returns true the node gets a generic [graphics.gd/classdb/EditorNode3DGizmo] assigned and is added to this plugin's list of active gizmos.
	HasGizmo(for_node_3d Node3D.Instance) bool
	// Override this method to return a custom [graphics.gd/classdb/EditorNode3DGizmo] for the 3D nodes of your choice, return null for the rest of nodes. See also [Interface.HasGizmo].
	...
	// Override this method to add all the gizmo elements whenever a gizmo update is requested. It's common to call [graphics.gd/classdb/EditorNode3DGizmo.Instance.Clear] at the beginning of this method and then add visual elements depending on the node's properties.
	Redraw(gizmo EditorNode3DGizmo.Instance)
	// Override this method to provide gizmo's handle names. The 'secondary' argument is true when the requested handle is secondary (see [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles] for more information). Called for this plugin's active gizmos.
	GetHandleName(gizmo EditorNode3DGizmo.Instance, handle_id int, secondary bool) string
	// Override this method to return true whenever to given handle should be highlighted in the editor. The 'secondary' argument is true when the requested handle is secondary (see [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles] for more information). Called for this plugin's active gizmos.
	...
	//
	// The 'secondary' argument is true when the requested handle is secondary (see [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles] for more information).
	//
	...
	// Override this method to update the node's properties when the user drags a gizmo handle (previously added with [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles]). The provided 'screen_pos' is the mouse position in screen coordinates and the 'camera' can be used to convert it to raycasts.
	//
	// The 'secondary' argument is true when the edited handle is secondary (see [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles] for more information).
	//
	// Called for this plugin's active gizmos.
	SetHandle(gizmo EditorNode3DGizmo.Instance, handle_id int, secondary bool, camera Camera3D.Instance, screen_pos Vector2.XY)
	// Override this method to commit a handle being edited (handles must have been previously added by [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles] during [Interface.Redraw]). This usually means creating an [graphics.gd/classdb/UndoRedo] action for the change, using the current handle value as "do" and the 'restore' argument as "undo".
	//
	// If the 'cancel' argument is true, the 'restore' value should be directly set, without any [graphics.gd/classdb/UndoRedo] action.
	//
	// The 'secondary' argument is true when the committed handle is secondary (see [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles] for more information).
	//
	// Called for this plugin's active gizmos.
	CommitHandle(gizmo EditorNode3DGizmo.Instance, handle_id int, secondary bool, restore any, cancel bool)
	// Override this method to allow selecting subgizmos using mouse clicks. Given a 'camera' and a 'screen_pos' in screen coordinates, this method should return which subgizmo should be selected. The returned value should be a unique subgizmo identifier, which can have any non-negative value and will be used in other virtual methods like [Interface.GetSubgizmoTransform] or [Interface.CommitSubgizmos]. Called for this plugin's active gizmos.
	SubgizmosIntersectRay(gizmo EditorNode3DGizmo.Instance, camera Camera3D.Instance, screen_pos Vector2.XY) int
	// Override this method to allow selecting subgizmos using mouse drag box selection. Given a 'camera' and 'frustum_planes', this method should return which subgizmos are contained within the frustums. The 'frustum_planes' argument consists of an array with all the [Plane.NormalD]s that make up the selection frustum. The returned value should contain a list of unique subgizmo identifiers, these identifiers can have any non-negative value and will be used in other virtual methods like [Interface.GetSubgizmoTransform] or [Interface.CommitSubgizmos]. Called for this plugin's active gizmos.
	SubgizmosIntersectFrustum(gizmo EditorNode3DGizmo.Instance, camera Camera3D.Instance, frustum_planes []Plane.NormalD) []int32
	// Override this method to return the current transform of a subgizmo. As with all subgizmo methods, the transform should be in local space respect to the gizmo's Node3D. This transform will be requested at the start of an edit and used in the restore argument in [Interface.CommitSubgizmos]. Called for this plugin's active gizmos.
	GetSubgizmoTransform(gizmo EditorNode3DGizmo.Instance, subgizmo_id int) Transform3D.BasisOrigin
	// Override this method to update the node properties during subgizmo editing (see [Interface.SubgizmosIntersectRay] and [Interface.SubgizmosIntersectFrustum]). The 'transform' is given in the Node3D's local coordinate system. Called for this plugin's active gizmos.
	...
	//
	// If the 'cancel' argument is true, the 'restores' transforms should be directly set, without any [graphics.gd/classdb/UndoRedo] action. As with all subgizmo methods, transforms are given in local space respect to the gizmo's Node3D. Called for this plugin's active gizmos.
...
/*
Creates an unshaded material with its variants (selected and/or editable) and adds them to the internal material list. They can then be accessed with [Instance.GetMaterial] and used in [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddMesh] and [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddLines]. Should not be overridden.
*/
...
/*
Creates an icon material with its variants (selected and/or editable) and adds them to the internal material list. They can then be accessed with [Instance.GetMaterial] and used in [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddUnscaledBillboard]. Should not be overridden.
*/
...
/*
Creates a handle material with its variants (selected and/or editable) and adds them to the internal material list. They can then be accessed with [Instance.GetMaterial] and used in [graphics.gd/classdb/EditorNode3DGizmo.Instance.AddHandles]. Should not be overridden.
...
/*
Adds a new material to the internal material list for the plugin. It can then be accessed with [Instance.GetMaterial]. Should not be overridden.
*/
...
/*
Gets material from the internal list of materials. If an [graphics.gd/classdb/EditorNode3DGizmo] is provided, it will try to get the corresponding variant (selected and/or editable).
*/
...
func (self class) CreateMaterial(name String.Readable, color Color.RGBA, billboard bool, on_top bool, use_vertex_color bool) { //gd:EditorNode3DGizmoPlugin.create_material
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.create_material, 0|(gdextension.SizeString<<4)|(gdextension.SizeColor<<8)|(gdextension.SizeBool<<12)|(gdextension.SizeBool<<16)|(gdextension.SizeBool<<20), &struct {
...
func (self class) CreateIconMaterial(name String.Readable, texture [1]gdclass.Texture2D, on_top bool, color Color.RGBA) { //gd:EditorNode3DGizmoPlugin.create_icon_material
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.create_icon_material, 0|(gdextension.SizeString<<4)|(gdextension.SizeObject<<8)|(gdextension.SizeBool<<12)|(gdextension.SizeColor<<16), &struct {
		name    gdextension.String
...
func (self class) CreateHandleMaterial(name String.Readable, billboard bool, texture [1]gdclass.Texture2D) { //gd:EditorNode3DGizmoPlugin.create_handle_material
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.create_handle_material, 0|(gdextension.SizeString<<4)|(gdextension.SizeBool<<8)|(gdextension.SizeObject<<12), &struct {
		name      gdextension.String
		billboard bool
		texture   gdextension.Object
...
func (self class) AddMaterial(name String.Readable, material [1]gdclass.StandardMaterial3D) { //gd:EditorNode3DGizmoPlugin.add_material
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_material, 0|(gdextension.SizeString<<4)|(gdextension.SizeObject<<8), &struct {
		name     gdextension.String
...
func (self class) GetMaterial(name String.Readable, gizmo [1]gdclass.EditorNode3DGizmo) [1]gdclass.StandardMaterial3D { //gd:EditorNode3DGizmoPlugin.get_material
	var r_ret = gdextension.Call[gdextension.Object](gd.ObjectChecked(self.AsObject()), methods.get_material, gdextension.SizeObject|(gdextension.SizeString<<4)|(gdextension.SizeObject<<8), &struct {
		name  gdextension.String
