...
/*
The default use of [graphics.gd/classdb/AcceptDialog] is to allow it to only be accepted or closed, with the same result. However, the [Instance.OnConfirmed] and [Instance.OnCanceled] signals allow to make the two actions different, and the [Instance.AddButton] method allows to add custom buttons and actions.
*/
...
/*
Returns the OK [graphics.gd/classdb/Button] instance.
...
You can use [Instance.RemoveButton] method to remove a button created with this method from the dialog.
*/
...
/*
Removes the 'button' from the dialog. Does NOT free the 'button'. The 'button' must be a [graphics.gd/classdb/Button] added with [Instance.AddButton] or [Instance.AddCancelButton] method. After removal, pressing the 'button' will no longer emit this dialog's [Instance.OnCustomAction] or [Instance.OnCanceled] signals.
*/
...
/*
Registers a [graphics.gd/classdb/LineEdit] in the dialog. When the enter key is pressed, the dialog will be accepted.
*/
