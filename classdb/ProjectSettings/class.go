...
/*
Stores variables that can be accessed from everywhere. Use [Instance.GetSetting], [Instance.SetSetting] or [Instance.HasSetting] to access them. Variables stored in project.godot are also loaded into [ProjectSettings], making this object very useful for reading custom game configuration options.
...
Note: This method doesn't take potential feature overrides into account automatically. Use [Instance.GetSettingWithOverride] to handle seamlessly.
*/
...
/*
Similar to [Instance.GetSetting], but applies feature tag overrides if any exists and is valid.
Example: If the setting override "application/config/name.windows" exists, and the following code is executed on a [i]Windows[/i] operating system, the overridden setting is printed instead:
...
Note: This is just a hint to display to the user that the editor must be restarted for changes to take effect. Enabling [Instance.SetRestartIfChanged] does [i]not[/i] delay the setting being set when changed.
*/
...
/*
Returns the localized path (starting with res://) corresponding to the absolute, native OS 'path'. See also [Instance.GlobalizePath].
*/
...
Note: This method is intended to be used by editor plugins, as modified [ProjectSettings] can't be loaded back in the running app. If you want to change project settings in exported projects, use [Instance.SaveCustom] to save override.cfg file.
*/
