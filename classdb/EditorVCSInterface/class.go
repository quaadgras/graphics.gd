...
/*
Defines the API that the editor uses to extract information from the underlying VCS. The implementation of this API is included in VCS plugins, which are GDExtension plugins that inherit [graphics.gd/classdb/EditorVCSInterface] and are attached (on demand) to the singleton instance of [graphics.gd/classdb/EditorVCSInterface]. Instead of performing the task themselves, all the virtual functions listed below are calling the internally overridden functions in the VCS plugins to provide a plug-n-play experience. A custom VCS plugin is supposed to inherit from [graphics.gd/classdb/EditorVCSInterface] and override each of these virtual functions.
*/
...
type Interface interface {
	...
	// Returns an slice of data structure items (see [Instance.CreateStatusFile]), each containing the status data of every modified file in the project folder.
	...
	Fetch(remote string)
	// Returns an slice of data structure items (see [Instance.CreateDiffHunk]), each containing a line diff between a file at 'file_path' and the 'text' which is passed in.
