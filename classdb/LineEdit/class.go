...
- When the [graphics.gd/classdb/LineEdit] control is focused using the keyboard arrow keys, it will only gain focus and not enter edit mode.

- To enter edit mode, click on the control with the mouse, see also [Instance.KeepEditingOnTextSubmit].
...
[graphics.gd/classdb/LineEdit] features many built-in shortcuts that are always available (Ctrl here maps to Cmd on macOS):
...
Note: Caret movement shortcuts listed above are not affected by [Instance.ShortcutKeysEnabled].
*/
package LineEdit
...
/*
Allows entering edit mode whether the [graphics.gd/classdb/LineEdit] is focused or not.

See also [Instance.KeepEditingOnTextSubmit].
*/
...
/*
Returns whether the [graphics.gd/classdb/LineEdit] is being edited.
*/
...
/*
Erases the [graphics.gd/classdb/LineEdit]'s [Instance.Text].
*/
...
/*
Inserts 'text' at the caret. If the resulting value is longer than [Instance.MaxLength], nothing happens.
*/
...
/*
Deletes a section of the [Instance.Text] going from position 'from_column' to 'to_column'. Both parameters should be within the text's length.
*/
...
const (
	...
	//
	// Non-printable escape characters are automatically stripped from the OS clipboard via [graphics.gd/classdb/String.Instance.StripEscapes].
	MenuPaste MenuItems = 2
	// Erases the whole [graphics.gd/classdb/LineEdit] text.
	MenuClear MenuItems = 3
	// Selects the whole [graphics.gd/classdb/LineEdit] text.
