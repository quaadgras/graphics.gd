...
/*
A [Texture2D] based on an [Image]. For an image to be displayed, an [ImageTexture] has to be created from it using the [Instance.CreateFromImage] method:
[codeblock]
...
Warning: Prefer to load imported textures with [Instance.@Gdscript.Load] over loading them from within the filesystem dynamically with [Instance.Image.Load], as it may not work in exported projects:
[codeblock]
var texture = load("res://icon.svg")
$Sprite2D.texture = texture
[/codeblock]
This is because images have to be imported as a [CompressedTexture2D] first to be loaded with [Instance.@Gdscript.Load]. If you'd still like to load an image file just like any other [Resource], import it as an [Image] resource instead, and then load it normally using the [Instance.@Gdscript.Load] method.
Note: The image can be retrieved from an imported texture using the [Instance.Texture2d.GetImage] method, which returns a copy of the image:
[codeblock]
var texture = load("res://icon.svg")
...
Note: The texture has to be created using [Instance.CreateFromImage] or initialized first with the [Instance.SetImage] method before it can be updated. The new image dimensions, format, and mipmaps configuration should match the existing texture's image configuration.
