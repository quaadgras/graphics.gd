...
To use [AStarGrid2D], you only need to set the [member region] of the grid, optionally set the [member cell_size], and then call the [Instance.Update] method:
...
/*
Returns true if the 'x' and 'y' is a valid grid coordinate (id), i.e. if it is inside [member region]. Equivalent to region.has_point(Vector2i(x, y)).
*/
...
/*
Returns true if the 'id' vector is a valid grid coordinate, i.e. if it is inside [member region]. Equivalent to region.has_point(id).
*/
...
/*
Updates the internal state of the grid according to the parameters to prepare it to search the path. Needs to be called if parameters like [member region], [member cell_size] or [member offset] are changed. [Instance.IsDirty] will return true if this is the case and this needs to be called.
