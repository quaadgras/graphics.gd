...
Input events are propagated through the [SceneTree] from the root node to all child nodes by calling [Instance.Node.Input]. For UI elements specifically, it makes more sense to override the virtual method [Instance.GuiInput], which filters out unrelated input events, such as by checking z-order, [member mouse_filter], focus, or if the event was inside of the control's bounding box.
Call [Instance.AcceptEvent] so no other node receives the event. Once you accept an input, it becomes handled so [Instance.Node.UnhandledInput] will not process it.
Only one [Control] node can be in focus. Only the node in focus will receive events. To get the focus, call [Instance.GrabFocus]. [Control] nodes lose focus when another node grabs it, or if you hide the node in focus.
Sets [member mouse_filter] to [MouseFilterIgnore] to tell a [Control] node to ignore mouse or touch events. You'll need it if you place an icon on top of a button.
...
Note: Theme items are [i]not[/i] [Object] properties. This means you can't access their values using [Instance.Object.Get] and [Instance.Object.Set]. Instead, use the get_theme_* and add_theme_*_override methods provided by this class.
*/
package Control
...
Note: If this method returns an empty [String] and [Instance.MakeCustomTooltip] is not overridden, no tooltip is displayed.
*/
...
/*
Godot calls this method to get data that can be dragged and dropped onto controls that expect drop data. Returns null if there is no data to drag. Controls that want to receive drop data should implement [Instance.CanDropData] and [Instance.DropData]. 'at_position' is local to this control. Drag may be forced with [Instance.ForceDrag].
...
/*
Godot calls this method to test if 'data' from a control's [Instance.GetDragData] can be dropped at 'at_position'. 'at_position' is local to this control.
This method should only be used to test the data. Process the data in [Instance.DropData].
...
/*
Godot calls this method to pass you the 'data' from a control's [Instance.GetDragData] result. Godot first calls [Instance.CanDropData] to test if 'data' is allowed to drop at 'at_position' where 'at_position' is local to this control.
...
/*
Virtual method to be implemented by the user. Returns a [Control] node that should be used as a tooltip instead of the default one. 'for_text' is the return value of [Instance.GetTooltip].
The returned node must be of type [Control] or Control-derived. It can have child nodes of any type. It is freed when the tooltip disappears, so make sure you always provide a new instance (if you want to use a pre-existing node from your scene tree, you can duplicate it and pass the duplicated instance). When null or a non-Control node is returned, the default tooltip will be used instead.
The returned node will be added as child to a [PopupPanel], so you should only provide the contents of that panel. That [PopupPanel] can be themed using [Instance.Theme.SetStylebox] for the type "TooltipPanel" (see [member tooltip_text] for an example).
...
- the control's [member mouse_filter] is set to [MouseFilterIgnore];
- the control is obstructed by another control on top, that doesn't have [member mouse_filter] set to [MouseFilterIgnore];
- the control's parent has [member mouse_filter] set to [MouseFilterStop] or has accepted the event;
- the control's parent has [member clip_contents] enabled and the 'event”s position is outside the parent's rectangle;
- the 'event”s position is outside the control (see [Instance.HasPoint]).
...
/*
Marks an input event as handled. Once you accept an input event, it stops propagating, even to nodes listening to [Instance.Node.UnhandledInput] or [Instance.Node.UnhandledKeyInput].
...
/*
Returns combined minimum size from [member custom_minimum_size] and [Instance.GetMinimumSize].
*/
...
/*
Sets the anchor for the specified [Side] to 'anchor'. A setter method for [member anchor_bottom], [member anchor_left], [member anchor_right] and [member anchor_top].
...
/*
Sets the [member position] to given 'position'.
...
/*
Sets the [member global_position] to given 'position'.
If 'keep_offsets' is true, control's anchors will be updated instead of offsets.
*/
...
Note: Using this method together with [Instance.Callable.CallDeferred] makes it more reliable, especially when called inside [Instance.Node.Ready].
*/
...
/*
Creates a local override for a theme [StyleBox] with the specified 'name'. Local overrides always take precedence when fetching theme items for the control. An override can be removed with [Instance.RemoveThemeStyleboxOverride].
...
/*
Creates a local override for a theme [Font] with the specified 'name'. Local overrides always take precedence when fetching theme items for the control. An override can be removed with [Instance.RemoveThemeFontOverride].
...
/*
Returns a [Color] from the first matching [Theme] in the tree if that [Theme] has a color item with the specified 'name' and 'theme_type'. If 'theme_type' is omitted the class name of the current control is used as the type, or [member theme_type_variation] if it is defined. If the type is a class name its parent classes are also checked, in order of inheritance. If the type is a variation its base types are checked, in order of dependency, then the control's class name and its parent classes are checked.
For the current control its local overrides are considered first (see [Instance.AddThemeColorOverride]), then its assigned [member theme]. After the current control, each parent control and its assigned [member theme] are considered; controls without a [member theme] assigned are skipped. If no matching [Theme] is found in the tree, the custom project [Theme] (see [member ProjectSettings.gui/theme/custom]) and the default [Theme] are used (see [ThemeDB]).
...
/*
Returns the tooltip text for the position 'at_position' in control's local coordinates, which will typically appear when the cursor is resting over this control. By default, it returns [member tooltip_text].
This method can be overridden to customize its behavior. See [Instance.GetTooltip].
Note: If this method returns an empty [String] and [Instance.MakeCustomTooltip] is not overridden, no tooltip is displayed.
*/
...
/*
Forces drag and bypasses [Instance.GetDragData] and [Instance.SetDragPreview] by passing 'data' and 'preview'. Drag will start even if the mouse is neither over nor pressed on this control.
The methods [Instance.CanDropData] and [Instance.DropData] must be implemented on controls that want to receive drop data.
*/
...
- 'drag_func' corresponds to [Instance.GetDragData] and requires a [Vector2];
- 'can_drop_func' corresponds to [Instance.CanDropData] and requires both a [Vector2] and a [Variant];
- 'drop_func' corresponds to [Instance.DropData] and requires both a [Vector2] and a [Variant].
*/
...
/*
Shows the given control at the mouse pointer. A good time to call this method is in [Instance.GetDragData]. The control must not be in the scene tree. You should not free the control, and you should not keep a reference to the control beyond the duration of the drag. It will be deleted automatically after the drag has ended.
...
/*
Returns true if a drag operation is successful. Alternative to [Instance.Viewport.GuiIsDragSuccessful].
...
/*
Moves the mouse cursor to 'position', relative to [member position] of this [Control].
...
/*
Invalidates the size cache in this node and in parent nodes up to top level. Intended to be used with [Instance.GetMinimumSize] when the return value is changed. Setting [member custom_minimum_size] directly calls this method automatically.
*/
...
/*
Returns the anchor for the specified [Side]. A getter method for [member anchor_bottom], [member anchor_left], [member anchor_right] and [member anchor_top].
*/
//go:nosplit
...
/*
Sets the offset for the specified [Side] to 'offset'. A setter method for [member offset_bottom], [member offset_left], [member offset_right] and [member offset_top].
*/
//go:nosplit
...
/*
Returns the offset for the specified [Side]. A getter method for [member offset_bottom], [member offset_left], [member offset_right] and [member offset_top].
*/
//go:nosplit
...
/*
Sets the focus neighbor for the specified [Side] to the [Control] at 'neighbor' node path. A setter method for [member focus_neighbor_bottom], [member focus_neighbor_left], [member focus_neighbor_right] and [member focus_neighbor_top].
*/
//go:nosplit
...
/*
Returns the focus neighbor for the specified [Side]. A getter method for [member focus_neighbor_bottom], [member focus_neighbor_left], [member focus_neighbor_right] and [member focus_neighbor_top].
Note: To find the next [Control] on the specific [Side], even if a neighbor is not assigned, use [Instance.FindValidFocusNeighbor].
*/
//go:nosplit
