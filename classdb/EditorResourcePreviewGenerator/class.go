...
Care must be taken because this function is always called from a thread (not the main thread).
'metadata' dictionary can be modified to store file-specific metadata that can be used in [Instance.Editorresourcetooltipplugin.MakeTooltipForPath] (like image size, sample length etc.).
*/
...
/*
Generate a preview directly from a path with the specified size. Implementing this is optional, as default code will load and call [Instance.Generate].
Returning null is an OK way to fail and let another generator take care.
Care must be taken because this function is always called from a thread (not the main thread).
'metadata' dictionary can be modified to store file-specific metadata that can be used in [Instance.Editorresourcetooltipplugin.MakeTooltipForPath] (like image size, sample length etc.).
*/
...
/*
If this function returns true, the generator will automatically generate the small previews from the normal preview texture generated by the methods [Instance.Generate] or [Instance.GenerateFromPath].
...
/*
If this function returns true, the generator will call [Instance.Generate] or [Instance.GenerateFromPath] for small previews as well.
By default, it returns false.
*/
