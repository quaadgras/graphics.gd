...
type Interface interface {
	// Stops playback. May be called multiple times before [Interface.Play], or in response to [graphics.gd/classdb/VideoStreamPlayer.Instance.Stop]. [Interface.IsPlaying] should return false once stopped.
	Stop()
	// Called in response to [graphics.gd/classdb/VideoStreamPlayer.Instance.Autoplay] or [graphics.gd/classdb/VideoStreamPlayer.Instance.Play]. Note that manual playback may also invoke [Interface.Stop] multiple times before this method is called. [Interface.IsPlaying] should return true once playing.
	Play()
	// Returns the playback state, as determined by calls to [Interface.Play] and [Interface.Stop].
	IsPlaying() bool
	// Set the paused status of video playback. [Interface.IsPaused] must return 'paused'. Called in response to the [graphics.gd/classdb/VideoStreamPlayer.Instance.Paused] setter.
	SetPaused(paused bool)
	// Returns the paused status, as set by [Interface.SetPaused].
	IsPaused() bool
	// Returns the video duration in seconds, if known, or 0 if unknown.
	GetLength() Float.X
	// Return the current playback timestamp. Called in response to the [graphics.gd/classdb/VideoStreamPlayer.Instance.StreamPosition] getter.
	GetPlaybackPosition() Float.X
	// Seeks to 'time' seconds. Called in response to the [graphics.gd/classdb/VideoStreamPlayer.Instance.StreamPosition] setter.
	Seek(time Float.X)
	// Select the audio track 'idx'. Called when playback starts, and in response to the [graphics.gd/classdb/VideoStreamPlayer.Instance.AudioTrack] setter.
	SetAudioTrack(idx int)
	// Allocates a [graphics.gd/classdb/Texture2D] in which decoded video frames will be drawn.
	GetTexture() Texture2D.Instance
	// Ticks video playback for 'delta' seconds. Called every frame as long as both [Interface.IsPaused] and [Interface.IsPlaying] return true.
...
/*
Render 'num_frames' audio frames (of [Interface.GetChannels] floats each) from 'buffer', starting from index 'offset' in the array. Returns the number of audio frames rendered, or -1 on error.
*/
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.VideoStreamPlayback](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.VideoStreamPlayback{pointers.Add[gdclass.VideoStreamPlayback]([3]uint64{})})
	...
	casted := Instance([1]gdclass.VideoStreamPlayback{pointers.New[gdclass.VideoStreamPlayback]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func (self class) MixAudio(num_frames int64, buffer Packed.Array[float32], offset int64) int64 { //gd:VideoStreamPlayback.mix_audio
	var r_ret = gdextension.Call[int64](gd.ObjectChecked(self.AsObject()), methods.mix_audio, gdextension.SizeInt|(gdextension.SizeInt<<4)|(gdextension.SizePackedArray<<8)|(gdextension.SizeInt<<12), &struct {
		num_frames int64
...
func (self class) AsVideoStreamPlayback() Advanced {
	return Advanced{pointers.AsA[gdclass.VideoStreamPlayback](self[0])}
}
func (self Instance) AsVideoStreamPlayback() Instance {
	return Instance{pointers.AsA[gdclass.VideoStreamPlayback](self[0])}
}
...
func init() {
	gdclass.Register("VideoStreamPlayback", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.VideoStreamPlayback](ptr)} })
}
