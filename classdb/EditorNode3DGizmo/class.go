...
/*
Override this method to return the current value of a handle. This value will be requested at the start of an edit and used as the restore argument in [Instance.CommitHandle].
The 'secondary' argument is true when the requested handle is secondary (see [Instance.AddHandles] for more information).
*/
...
/*
Override this method to commit a handle being edited (handles must have been previously added by [Instance.AddHandles]). This usually means creating an [UndoRedo] action for the change, using the current handle value as "do" and the 'restore' argument as "undo".
...
/*
Override this method to allow selecting subgizmos using mouse clicks. Given a 'camera' and a 'point' in screen coordinates, this method should return which subgizmo should be selected. The returned value should be a unique subgizmo identifier, which can have any non-negative value and will be used in other virtual methods like [Instance.GetSubgizmoTransform] or [Instance.CommitSubgizmos].
*/
...
/*
Override this method to update the node properties during subgizmo editing (see [Instance.SubgizmosIntersectRay] and [Instance.SubgizmosIntersectFrustum]). The 'transform' is given in the [Node3D]'s local coordinate system.
*/
...
/*
Override this method to return the current transform of a subgizmo. This transform will be requested at the start of an edit and used as the restore argument in [Instance.CommitSubgizmos].
*/
...
/*
Override this method to commit a group of subgizmos being edited (see [Instance.SubgizmosIntersectRay] and [Instance.SubgizmosIntersectFrustum]). This usually means creating an [UndoRedo] action for the change, using the current transforms as "do" and the 'restores' transforms as "undo".
...
There are virtual methods which will be called upon editing of these handles. Call this method during [Instance.Redraw].
*/
...
/*
Returns the [EditorNode3DGizmoPlugin] that owns this gizmo. It's useful to retrieve materials using [Instance.Editornode3dgizmoplugin.GetMaterial].
*/
//go:nosplit
