...
/*
Gizmo that is used for providing custom visualization and editing (handles and subgizmos) for [graphics.gd/classdb/Node3D] objects. Can be overridden to create custom gizmos, but for simple gizmos creating a [graphics.gd/classdb/EditorNode3DGizmoPlugin] is usually recommended.
*/
...
type Interface interface {
	// Override this method to add all the gizmo elements whenever a gizmo update is requested. It's common to call [Instance.Clear] at the beginning of this method and then add visual elements depending on the node's properties.
	Redraw()
	// Override this method to return the name of an edited handle (handles must have been previously added by [Instance.AddHandles]). Handles can be named for reference to the user when editing.
	//
	// The 'secondary' argument is true when the requested handle is secondary (see [Instance.AddHandles] for more information).
	...
	// Override this method to return the current value of a handle. This value will be requested at the start of an edit and used as the restore argument in [Interface.CommitHandle].
	//
	// The 'secondary' argument is true when the requested handle is secondary (see [Instance.AddHandles] for more information).
	GetHandleValue(id int, secondary bool) any
	BeginHandleAction(id int, secondary bool)
	// Override this method to update the node properties when the user drags a gizmo handle (previously added with [Instance.AddHandles]). The provided 'point' is the mouse position in screen coordinates and the 'camera' can be used to convert it to raycasts.
	//
	// The 'secondary' argument is true when the edited handle is secondary (see [Instance.AddHandles] for more information).
	SetHandle(id int, secondary bool, camera Camera3D.Instance, point Vector2.XY)
	// Override this method to commit a handle being edited (handles must have been previously added by [Instance.AddHandles]). This usually means creating an [graphics.gd/classdb/UndoRedo] action for the change, using the current handle value as "do" and the 'restore' argument as "undo".
	//
	// If the 'cancel' argument is true, the 'restore' value should be directly set, without any [graphics.gd/classdb/UndoRedo] action.
	//
	// The 'secondary' argument is true when the committed handle is secondary (see [Instance.AddHandles] for more information).
	CommitHandle(id int, secondary bool, restore any, cancel bool)
	// Override this method to allow selecting subgizmos using mouse clicks. Given a 'camera' and a 'point' in screen coordinates, this method should return which subgizmo should be selected. The returned value should be a unique subgizmo identifier, which can have any non-negative value and will be used in other virtual methods like [Interface.GetSubgizmoTransform] or [Interface.CommitSubgizmos].
	...
	// Override this method to update the node properties during subgizmo editing (see [Interface.SubgizmosIntersectRay] and [Interface.SubgizmosIntersectFrustum]). The 'transform' is given in the [graphics.gd/classdb/Node3D]'s local coordinate system.
	SetSubgizmoTransform(id int, transform Transform3D.BasisOrigin)
	// Override this method to return the current transform of a subgizmo. This transform will be requested at the start of an edit and used as the restore argument in [Interface.CommitSubgizmos].
	GetSubgizmoTransform(id int) Transform3D.BasisOrigin
	// Override this method to commit a group of subgizmos being edited (see [Interface.SubgizmosIntersectRay] and [Interface.SubgizmosIntersectFrustum]). This usually means creating an [graphics.gd/classdb/UndoRedo] action for the change, using the current transforms as "do" and the 'restores' transforms as "undo".
	//
	// If the 'cancel' argument is true, the 'restores' transforms should be directly set, without any [graphics.gd/classdb/UndoRedo] action.
...
func (self class) AddLines(lines Packed.Array[Vector3.XYZ], material [1]gdclass.Material, billboard bool, modulate Color.RGBA) { //gd:EditorNode3DGizmo.add_lines
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_lines, 0|(gdextension.SizePackedArray<<4)|(gdextension.SizeObject<<8)|(gdextension.SizeBool<<12)|(gdextension.SizeColor<<16), &struct {
...
func (self class) AddMesh(mesh [1]gdclass.Mesh, material [1]gdclass.Material, transform Transform3D.BasisOrigin, skeleton [1]gdclass.SkinReference) { //gd:EditorNode3DGizmo.add_mesh
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_mesh, 0|(gdextension.SizeObject<<4)|(gdextension.SizeObject<<8)|(gdextension.SizeTransform3D<<12)|(gdextension.SizeObject<<16), &struct {
...
func (self class) AddCollisionSegments(segments Packed.Array[Vector3.XYZ]) { //gd:EditorNode3DGizmo.add_collision_segments
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_collision_segments, 0|(gdextension.SizePackedArray<<4), &struct {
...
func (self class) AddCollisionTriangles(triangles [1]gdclass.TriangleMesh) { //gd:EditorNode3DGizmo.add_collision_triangles
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_collision_triangles, 0|(gdextension.SizeObject<<4), &struct{ triangles gdextension.Object }{gdextension.Object(gd.ObjectChecked(triangles[0].AsObject()))})
}
...
func (self class) AddUnscaledBillboard(material [1]gdclass.Material, default_scale float64, modulate Color.RGBA) { //gd:EditorNode3DGizmo.add_unscaled_billboard
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_unscaled_billboard, 0|(gdextension.SizeObject<<4)|(gdextension.SizeFloat<<8)|(gdextension.SizeColor<<12), &struct {
		material      gdextension.Object
...
func (self class) AddHandles(handles Packed.Array[Vector3.XYZ], material [1]gdclass.Material, ids Packed.Array[int32], billboard bool, secondary bool) { //gd:EditorNode3DGizmo.add_handles
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_handles, 0|(gdextension.SizePackedArray<<4)|(gdextension.SizeObject<<8)|(gdextension.SizePackedArray<<12)|(gdextension.SizeBool<<16)|(gdextension.SizeBool<<20), &struct {
...
func (self class) SetNode3d(node [1]gdclass.Node) { //gd:EditorNode3DGizmo.set_node_3d
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.set_node_3d, 0|(gdextension.SizeObject<<4), &struct{ node gdextension.Object }{gdextension.Object(gd.PointerWithOwnershipTransferredToGodot(node[0].AsObject()[0]))})
}
...
func (self class) GetNode3d() [1]gdclass.Node3D { //gd:EditorNode3DGizmo.get_node_3d
	var r_ret = gdextension.Call[gdextension.Object](gd.ObjectChecked(self.AsObject()), methods.get_node_3d, gdextension.SizeObject, &struct{}{})
	var ret = [1]gdclass.Node3D{gd.PointerLifetimeBoundTo[gdclass.Node3D](self.AsObject(), r_ret)}
	return ret
}

/*
Returns the [graphics.gd/classdb/EditorNode3DGizmoPlugin] that owns this gizmo. It's useful to retrieve materials using [graphics.gd/classdb/EditorNode3DGizmoPlugin.Instance.GetMaterial].
*/
//go:nosplit
func (self class) GetPlugin() [1]gdclass.EditorNode3DGizmoPlugin { //gd:EditorNode3DGizmo.get_plugin
	var r_ret = gdextension.Call[gdextension.Object](gd.ObjectChecked(self.AsObject()), methods.get_plugin, gdextension.SizeObject, &struct{}{})
...
func (self class) SetHidden(hidden bool) { //gd:EditorNode3DGizmo.set_hidden
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.set_hidden, 0|(gdextension.SizeBool<<4), &struct{ hidden bool }{hidden})
}
...
func (self class) IsSubgizmoSelected(id int64) bool { //gd:EditorNode3DGizmo.is_subgizmo_selected
	var r_ret = gdextension.Call[bool](gd.ObjectChecked(self.AsObject()), methods.is_subgizmo_selected, gdextension.SizeBool|(gdextension.SizeInt<<4), &struct{ id int64 }{id})
	var ret = r_ret
	return ret
}
...
func (self class) GetSubgizmoSelection() Packed.Array[int32] { //gd:EditorNode3DGizmo.get_subgizmo_selection
	var r_ret = gdextension.Call[gd.PackedPointers](gd.ObjectChecked(self.AsObject()), methods.get_subgizmo_selection, gdextension.SizePackedArray, &struct{}{})
