...
/*
This class can be used to discover compatible [graphics.gd/classdb/UPNPDevice]s on the local network and execute commands on them, like managing port mappings (for port forwarding/NAT traversal) and querying the local and remote network IP address. Note that methods on this class are synchronous and block the calling thread.

To forward a specific port (here 7777, note both [Instance.Discover] and [Instance.AddPortMapping] can return errors that should be checked):
...
Note: UPnP discovery blocks the current thread. To perform discovery without blocking the main thread, use [graphics.gd/classdb/Thread]s like this:
...
/*
Returns the number of discovered [graphics.gd/classdb/UPNPDevice]s.
*/
...
/*
Returns the [graphics.gd/classdb/UPNPDevice] at the given 'index'.
*/
...
/*
Adds the given [graphics.gd/classdb/UPNPDevice] to the list of discovered devices.
*/
...
/*
Returns the default gateway. That is the first discovered [graphics.gd/classdb/UPNPDevice] that is also a valid IGD (InternetGatewayDevice).
*/
...
/*
Discovers local [graphics.gd/classdb/UPNPDevice]s. Clears the list of previously discovered devices.
...
/*
Returns the external [graphics.gd/classdb/IP] address of the default gateway (see [Instance.GetGateway]) as string. Returns an empty string on error.
*/
...
/*
Adds a mapping to forward the external 'port' (between 1 and 65535, although recommended to use port 1024 or above) on the default gateway (see [Instance.GetGateway]) to the 'port_internal' on the local machine for the given protocol 'proto' (either "TCP" or "UDP", with UDP being the default). If a port mapping for the given port and protocol combination already exists on that gateway device, this method tries to overwrite it. If that is not desired, you can retrieve the gateway manually with [Instance.GetGateway] and call [Instance.AddPortMapping] on it, if any. Note that forwarding a well-known port (below 1024) with UPnP may fail depending on the device.
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.UPNP](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.UPNP{pointers.Add[gdclass.UPNP]([3]uint64{})})
	...
	casted := Instance([1]gdclass.UPNP{pointers.New[gdclass.UPNP]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func (self class) AsUPNP() Advanced         { return Advanced{pointers.AsA[gdclass.UPNP](self[0])} }
func (self Instance) AsUPNP() Instance      { return Instance{pointers.AsA[gdclass.UPNP](self[0])} }
...
func init() {
	gdclass.Register("UPNP", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.UPNP](ptr)} })
}
...
const (
	...
	// Not authorized to use the command on the [graphics.gd/classdb/UPNPDevice]. May be returned when the user disabled UPNP on their router.
	UpnpResultNotAuthorized UPNPResult = 1
	// No port mapping was found for the given port, protocol combination on the given [graphics.gd/classdb/UPNPDevice].
	...
	// The [graphics.gd/classdb/UPNPDevice] does not allow wildcard values for the source IP address.
	UpnpResultSrcIpWildcardNotPermitted UPNPResult = 6
	// The [graphics.gd/classdb/UPNPDevice] does not allow wildcard values for the external port.
	UpnpResultExtPortWildcardNotPermitted UPNPResult = 7
	// The [graphics.gd/classdb/UPNPDevice] does not allow wildcard values for the internal port.
	...
	// No gateway available. You may need to call [Instance.Discover] first, or discovery didn't detect any valid IGDs (InternetGatewayDevices).
	UpnpResultNoGateway UPNPResult = 26
	// No devices available. You may need to call [Instance.Discover] first, or discovery didn't detect any valid [graphics.gd/classdb/UPNPDevice]s.
