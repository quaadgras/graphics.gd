...
To create a [TreeItem], use [Instance.Tree.CreateItem] or [Instance.Treeitem.CreateChild]. To remove a [TreeItem], use [Instance.Object.Free].
...
Note: This option only affects the type of control ([LineEdit] or [TextEdit]) that appears when editing the column. You can set multiline values with [Instance.SetText] even if the column is not multiline editable.
*/
...
/*
Sets the given column's custom draw callback. Use an empty [Callable] (Callable()) to clear the custom callback. The cell has to be in [CellModeCustom] to use this feature.
...
/*
Removes the given child [TreeItem] and all its children from the [Tree]. Note that it doesn't free the item from memory, so it can be reused later (see [Instance.AddChild]). To completely remove a [TreeItem] use [Instance.Object.Free].
Note: If you want to move a child from one [Tree] to another, then instead of removing and adding it manually you can use [Instance.MoveBefore] or [Instance.MoveAfter].
*/
