...
/*
Shape casting allows to detect collision objects by sweeping its [Instance.Shape] along the cast direction determined by [Instance.TargetPosition]. This is similar to [graphics.gd/classdb/RayCast2D], but it allows for sweeping a region of space, rather than just a straight line. [graphics.gd/classdb/ShapeCast2D] can detect multiple collision objects. It is useful for things like wide laser beams or snapping a simple shape to a floor.

Immediate collision overlaps can be done with the [Instance.TargetPosition] set to Vector2(0, 0) and by calling [Instance.ForceShapecastUpdate] within the same physics frame. This helps to overcome some limitations of [graphics.gd/classdb/Area2D] when used as an instantaneous detection area, as collision information isn't immediately available to it.
