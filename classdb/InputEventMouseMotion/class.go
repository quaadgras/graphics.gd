...
/*
Stores information about a mouse or a pen motion. This includes relative position, absolute position, and velocity. See [graphics.gd/classdb/Node.Instance.Input].

Note: By default, this event is only emitted once per frame rendered at most. If you need more precise input reporting, set [graphics.gd/classdb/Input.UseAccumulatedInput] to false to make events emitted as often as possible. If you use InputEventMouseMotion to draw lines, consider using [graphics.gd/classdb/Geometry2D.BresenhamLine] as well to avoid visible gaps in lines if the user is moving the mouse quickly.
