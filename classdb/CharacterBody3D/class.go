...
/*
[graphics.gd/classdb/CharacterBody3D] is a specialized class for physics bodies that are meant to be user-controlled. They are not affected by physics at all, but they affect other physics bodies in their path. They are mainly used to provide high-level API to move objects with wall and slope detection ([Instance.MoveAndSlide] method) in addition to the general collision detection provided by [graphics.gd/classdb/PhysicsBody3D.Instance.MoveAndCollide]. This makes it useful for highly configurable physics bodies that must move in specific ways and collide with the world, as is often the case with user-controlled characters.

For game objects that don't require complex movement or collision detection, such as moving platforms, [graphics.gd/classdb/AnimatableBody3D] is simpler to configure.
*/
...
/*
Moves the body based on [Instance.Velocity]. If the body collides with another, it will slide along the other body rather than stop immediately. If the other body is a [graphics.gd/classdb/CharacterBody3D] or [graphics.gd/classdb/RigidBody3D], it will also be affected by the motion of the other body. You can use this to make moving and rotating platforms, or to make nodes push other nodes.

Modifies [Instance.Velocity] if a slide collision occurred. To get the latest collision call [Instance.GetLastSlideCollision], for more detailed information about collisions that occurred, use [Instance.GetSlideCollision].
...
/*
Returns the last motion applied to the [graphics.gd/classdb/CharacterBody3D] during the last call to [Instance.MoveAndSlide]. The movement can be split into multiple motions when sliding occurs, and this method return the last one, which is useful to retrieve the current direction of the movement.
*/
...
/*
Returns a [graphics.gd/classdb/KinematicCollision3D], which contains information about the latest collision that occurred during the last call to [Instance.MoveAndSlide].
*/
