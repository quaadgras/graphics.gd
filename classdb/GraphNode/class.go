...
Note: While GraphNode is set up using slots and slot indices, connections are made between the ports which are enabled. Because of that [GraphEdit] uses the port's index and not the slot's index. You can use [Instance.GetInputPortSlot] and [Instance.GetOutputPortSlot] to get the slot index from the port index.
*/
package GraphNode
