...
/*
This [graphics.gd/classdb/SkeletonModifier3D] rotates a bone to look at a target. This is helpful for moving a character's head to look at the player, rotating a turret to look at a target, or any other case where you want to make a bone rotate towards something quickly and easily.

When applying multiple [graphics.gd/classdb/LookAtModifier3D]s, the [graphics.gd/classdb/LookAtModifier3D] assigned to the parent bone must be put above the [graphics.gd/classdb/LookAtModifier3D] assigned to the child bone in the list in order for the child bone results to be correct.
*/
...
/*
Returns whether the target is within the angle limitations. It is useful for unsetting the [Instance.TargetNode] when the target is outside of the angle limitations.

Note: The value is updated after [graphics.gd/classdb/SkeletonModifier3D.Instance.ProcessModification]. To retrieve this value correctly, we recommend using the signal [Instance.OnSkeletonmodifier3d.ModificationProcessed].
*/
...
const (
	// The bone rest position of the bone specified in [Instance.Bone] is used as origin.
	OriginFromSelf OriginFrom = 0
	// The bone global pose position of the bone specified in [Instance.OriginBone] is used as origin.
	//
	// Note: It is recommended that you select only the parent bone unless you are familiar with the bone processing process. The specified bone pose at the time the [graphics.gd/classdb/LookAtModifier3D] is processed is used as a reference. In other words, if you specify a child bone and the [graphics.gd/classdb/LookAtModifier3D] causes the child bone to move, the rendered result and direction will not match.
	OriginFromSpecificBone OriginFrom = 1
	// The global position of the [graphics.gd/classdb/Node3D] specified in [Instance.OriginExternalNode] is used as origin.
	//
	// Note: Same as [OriginFromSpecificBone], when specifying a [graphics.gd/classdb/BoneAttachment3D] with a child bone assigned, the rendered result and direction will not match.
