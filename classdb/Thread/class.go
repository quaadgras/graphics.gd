...
If the method takes some arguments, you can pass them using [Instance.Callable.Bind].
...
/*
Returns the current [Thread]'s ID, uniquely identifying it among all threads. If the [Thread] has not started running or if [Instance.WaitToFinish] has been called, this returns an empty string.
*/
...
/*
Returns true if this [Thread] has been started. Once started, this will return true until it is joined using [Instance.WaitToFinish]. For checking if a [Thread] is still executing its task, use [Instance.IsAlive].
*/
...
/*
Returns true if this [Thread] is currently running the provided function. This is useful for determining if [Instance.WaitToFinish] can be called without blocking the calling thread.
To check if a [Thread] is joinable, use [Instance.IsStarted].
*/
...
/*
Joins the [Thread] and waits for it to finish. Returns the output of the [Callable] passed to [Instance.Start].
