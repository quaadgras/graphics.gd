...
/*
A unit of execution in a process. Can run methods on [graphics.gd/classdb/Object]s simultaneously. The use of synchronization via [graphics.gd/classdb/Mutex] or [graphics.gd/classdb/Semaphore] is advised if working with shared objects.

Warning:

To ensure proper cleanup without crashes or deadlocks, when a [graphics.gd/classdb/Thread]'s reference count reaches zero and it is therefore destroyed, the following conditions must be met:

- It must not have any [graphics.gd/classdb/Mutex] objects locked.

- It must not be waiting on any [graphics.gd/classdb/Semaphore] objects.

- [Instance.WaitToFinish] should have been called on it.
*/
package Thread
...
/*
Starts a new [graphics.gd/classdb/Thread] that calls 'callable'.

If the method takes some arguments, you can pass them using [graphics.gd/classdb/Callable.Instance.Bind].

The 'priority' of the [graphics.gd/classdb/Thread] can be changed by passing a value from the [Priority] enum.
...
/*
Returns the current [graphics.gd/classdb/Thread]'s ID, uniquely identifying it among all threads. If the [graphics.gd/classdb/Thread] has not started running or if [Instance.WaitToFinish] has been called, this returns an empty string.
*/
...
/*
Returns true if this [graphics.gd/classdb/Thread] has been started. Once started, this will return true until it is joined using [Instance.WaitToFinish]. For checking if a [graphics.gd/classdb/Thread] is still executing its task, use [Instance.IsAlive].
*/
...
/*
Returns true if this [graphics.gd/classdb/Thread] is currently running the provided function. This is useful for determining if [Instance.WaitToFinish] can be called without blocking the calling thread.

To check if a [graphics.gd/classdb/Thread] is joinable, use [Instance.IsStarted].
*/
...
/*
Joins the [graphics.gd/classdb/Thread] and waits for it to finish. Returns the output of the func passed to [Instance.Start].

Should either be used when you want to retrieve the value returned from the method called by the [graphics.gd/classdb/Thread] or before freeing the instance that contains the [graphics.gd/classdb/Thread].

To determine if this can be called without blocking the calling thread, check if [Instance.IsAlive] is false.
*/
...
/*
Sets whether the thread safety checks the engine normally performs in methods of certain classes (e.g., [graphics.gd/classdb/Node]) should happen on the current thread.
...
Note: This is useful for scripts running on either arbitrary [graphics.gd/classdb/Thread] objects or tasks submitted to the [graphics.gd/classdb/WorkerThreadPool]. It doesn't apply to code running during [graphics.gd/classdb/Node] group processing, where the checks will be always performed.

Note: Even in the case of having disabled the checks in a [graphics.gd/classdb/WorkerThreadPool] task, there's no need to re-enable them at the end. The engine will do so.
*/
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.Thread](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.Thread{pointers.Add[gdclass.Thread]([3]uint64{})})
	...
	casted := Instance([1]gdclass.Thread{pointers.New[gdclass.Thread]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func init() {
	gdclass.Register("Thread", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.Thread](ptr)} })
}
