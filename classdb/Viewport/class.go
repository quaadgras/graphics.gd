...
Note: When trying to store the current texture (e.g. in a file), it might be completely black or outdated if used too early, especially when used in e.g. [Instance.Node.Ready]. To make sure the texture you get is correct, you can await [signal RenderingServer.frame_post_draw] signal.
[codeblock]
func _ready():
...
While this method serves a similar purpose as [Instance.Input.ParseInputEvent], it does not remap the specified 'event' based on project settings like [member ProjectSettings.input_devices/pointing/emulate_touch_from_mouse].
Calling this method will propagate calls to child nodes for following methods in the given order:
- [Instance.Node.Input]
- [Instance.Control.GuiInput] for [Control] nodes
- [Instance.Node.ShortcutInput]
- [Instance.Node.UnhandledKeyInput]
- [Instance.Node.UnhandledInput]
...
/*
Inform the Viewport that the mouse has entered its area. Use this function before sending an [InputEventMouseButton] or [InputEventMouseMotion] to the [Viewport] with [Instance.Viewport.PushInput]. See also [Instance.NotifyMouseExited].
...
Typically the leaf [Control] node or deepest level of the subtree which claims hover. This is very useful when used together with [Instance.Node.IsAncestorOf] to find if the mouse is within a control tree.
*/
...
/*
Returns whether the current [InputEvent] has been handled. Input events are not handled until [Instance.SetInputAsHandled] has been called during the lifetime of an [InputEvent].
This is usually done as part of input handling methods like [Instance.Node.Input], [Instance.Control.GuiInput] or others, as well as in corresponding signal handlers.
If [member handle_input_locally] is set to false, this method will try finding the first parent viewport that is set to handle input locally, and return its value for [Instance.IsInputHandled] instead.
*/
