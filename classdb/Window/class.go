...
const (
	// The window can't be resized by dragging its resize grip. It's still possible to resize the window using [Instance.Size]. This flag is ignored for full screen windows. Set with [Instance.Unresizable].
	FlagResizeDisabled Flags = 0
	// The window do not have native title bar and other decorations. This flag is ignored for full-screen windows. Set with [Instance.Borderless].
	FlagBorderless Flags = 1
	// The window is floating on top of all other windows. This flag is ignored for full-screen windows. Set with [Instance.AlwaysOnTop].
	FlagAlwaysOnTop Flags = 2
	// The window background can be transparent. Set with [Instance.Transparent].
	//
	// Note: This flag has no effect if either [graphics.gd/classdb/ProjectSettings] "display/window/per_pixel_transparency/allowed", or the window's [graphics.gd/classdb/Viewport.Instance.TransparentBg] is set to false.
	FlagTransparent Flags = 3
	// The window can't be focused. No-focus window will ignore all input, except mouse clicks. Set with [Instance.Unfocusable].
	FlagNoFocus Flags = 4
	// Window is part of menu or [graphics.gd/classdb/OptionButton] dropdown. This flag can't be changed when the window is visible. An active popup window will exclusively receive all input, without stealing focus from its parent. Popup windows are automatically closed when uses click outside it, or when an application is switched. Popup window must have transient parent set (see [Instance.Transient]).
	//
	// Note: This flag has no effect in embedded windows (unless said window is a [graphics.gd/classdb/Popup]).
	FlagPopup Flags = 5
	// Window content is expanded to the full size of the window. Unlike borderless window, the frame is left intact and can be used to resize the window, title bar is transparent, but have minimize/maximize/close buttons. Set with [Instance.ExtendToTitle].
	//
...
const (
	// The content will not be scaled to match the [graphics.gd/classdb/Window]'s size.
...
const (
	// Initial window position is determined by [Instance.Position].
	...
	// Initial window position is the center of [Instance.CurrentScreen] screen.
