...
Note: If the result of this function should change, use [Instance.NotifyRuntimeTileDataUpdate] to notify the [TileMapLayer] it needs an update.
*/
...
/*
Called with a [TileData] object about to be used internally by the [TileMapLayer], allowing its modification at runtime.
This method is only called if [Instance.UseTileDataRuntimeUpdate] is implemented and returns true for the given tile 'coords'.
Warning: The 'tile_data' object's sub-resources are the same as the one in the TileSet. Modifying them might impact the whole TileSet. Instead, make sure to duplicate those resources.
Note: If the properties of 'tile_data' object should change over time, use [Instance.NotifyRuntimeTileDataUpdate] to notify the [TileMapLayer] it needs an update.
*/
...
/*
Creates and returns a new [TileMapPattern] from the given array of cells. See also [Instance.SetPattern].
*/
...
/*
Pastes the [TileMapPattern] at the given 'position' in the tile map. See also [Instance.GetPattern].
*/
...
/*
Returns the coordinates of the tile for given physics body [RID]. Such an [RID] can be retrieved from [Instance.Kinematiccollision2d.GetColliderRid], when colliding with a tile.
*/
...
/*
Notifies the [TileMapLayer] node that calls to [Instance.UseTileDataRuntimeUpdate] or [Instance.TileDataRuntimeUpdate] will lead to different results. This will thus trigger a [TileMapLayer] update.
...
Note: This does not trigger a direct update of the [TileMapLayer], the update will be done at the end of the frame as usual (unless you call [Instance.UpdateInternals]).
*/
...
/*
Returns for the given coordinates 'coords_in_pattern' in a [TileMapPattern] the corresponding cell coordinates if the pattern was pasted at the 'position_in_tilemap' coordinates (see [Instance.SetPattern]). This mapping is required as in half-offset tile shapes, the mapping might not work by calculating position_in_tile_map + coords_in_pattern.
*/
...
/*
Returns the centered position of a cell in the [TileMapLayer]'s local coordinate space. To convert the returned value into global coordinates, use [Instance.Node2d.ToGlobal]. See also [Instance.LocalToMap].
...
By default this returns the default [World2D] navigation map, unless a custom map was provided using [Instance.SetNavigationMap].
*/
