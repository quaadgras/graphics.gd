...
/*
A 2D texture that obtains colors from a [graphics.gd/classdb/Gradient] to fill the texture data. This texture is able to transform a color transition into different patterns such as a linear or a radial gradient. The gradient is sampled individually for each pixel so it does not necessarily represent an exact copy of the gradient(see [Instance.Width] and [Instance.Height]). See also [graphics.gd/classdb/GradientTexture1D], [graphics.gd/classdb/CurveTexture] and [graphics.gd/classdb/CurveXYZTexture].
*/
