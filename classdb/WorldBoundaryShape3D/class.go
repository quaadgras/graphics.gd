...
/*
A 3D world boundary shape, intended for use in physics. [graphics.gd/classdb/WorldBoundaryShape3D] works like an infinite plane that forces all physics bodies to stay above it. The [Instance.Plane]'s normal determines which direction is considered as "above" and in the editor, the line over the plane represents this direction. It can for example be used for endless flat floors.

Note: When the physics engine is set to Jolt Physics in the project settings ([graphics.gd/classdb/ProjectSettings] "physics/3d/physics_engine"), [graphics.gd/classdb/WorldBoundaryShape3D] has a finite size (centered at the shape's origin). It can be adjusted by changing [graphics.gd/classdb/ProjectSettings] "physics/jolt_physics_3d/limits/world_boundary_shape_size".
*/
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.WorldBoundaryShape3D](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.WorldBoundaryShape3D{pointers.Add[gdclass.WorldBoundaryShape3D]([3]uint64{})})
	...
	casted := Instance([1]gdclass.WorldBoundaryShape3D{pointers.New[gdclass.WorldBoundaryShape3D]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func (self class) AsWorldBoundaryShape3D() Advanced {
	return Advanced{pointers.AsA[gdclass.WorldBoundaryShape3D](self[0])}
}
func (self Instance) AsWorldBoundaryShape3D() Instance {
	return Instance{pointers.AsA[gdclass.WorldBoundaryShape3D](self[0])}
}
...
func init() {
	gdclass.Register("WorldBoundaryShape3D", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.WorldBoundaryShape3D](ptr)} })
}
