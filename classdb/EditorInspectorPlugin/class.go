...
When an object is edited, the [Interface.CanHandle] function is called and must return true if the object type is supported.

If supported, the function [Interface.ParseBegin] will be called, allowing to place custom controls at the beginning of the class.

Subsequently, the [Interface.ParseCategory] and [Interface.ParseProperty] are called for every category and property. They offer the ability to add custom controls to the inspector too.

Finally, [Interface.ParseEnd] will be called.

On each of these calls, the "add" functions can be called.

To use [graphics.gd/classdb/EditorInspectorPlugin], register it using the [graphics.gd/classdb/EditorPlugin.Instance.AddInspectorPlugin] method first.
*/
...
type Interface interface {
	...
	// Called to allow adding property-specific editors to the property list for 'object'. The added editor control must extend [graphics.gd/classdb/EditorProperty]. Returning true removes the built-in editor for this property, otherwise allows to insert a custom editor before the built-in one.
...
/*
Adds a property editor for an individual property. The 'editor' control must extend [graphics.gd/classdb/EditorProperty].

There can be multiple property editors for a property. If 'add_to_end' is true, this newly added editor will be displayed after all the other editors of the property whose 'add_to_end' is false. For example, the editor uses this parameter to add an "Edit Region" button for [graphics.gd/classdb/Sprite2D.Instance.RegionRect] below the regular [Rect2.PositionSize] editor.
...
/*
Adds an editor that allows modifying multiple properties. The 'editor' control must extend [graphics.gd/classdb/EditorProperty].
*/
...
func (self class) AddCustomControl(control [1]gdclass.Control) { //gd:EditorInspectorPlugin.add_custom_control
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_custom_control, 0|(gdextension.SizeObject<<4), &struct{ control gdextension.Object }{gdextension.Object(gd.PointerWithOwnershipTransferredToGodot(control[0].AsObject()[0]))})
}
...
func (self class) AddPropertyEditor(property String.Readable, editor [1]gdclass.Control, add_to_end bool, label String.Readable) { //gd:EditorInspectorPlugin.add_property_editor
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_property_editor, 0|(gdextension.SizeString<<4)|(gdextension.SizeObject<<8)|(gdextension.SizeBool<<12)|(gdextension.SizeString<<16), &struct {
...
func (self class) AddPropertyEditorForMultipleProperties(label String.Readable, properties Packed.Strings, editor [1]gdclass.Control) { //gd:EditorInspectorPlugin.add_property_editor_for_multiple_properties
	gdextension.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.add_property_editor_for_multiple_properties, 0|(gdextension.SizeString<<4)|(gdextension.SizePackedArray<<8)|(gdextension.SizeObject<<12), &struct {
		label      gdextension.String
