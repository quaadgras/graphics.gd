...
To interact with the debugger, an instance of this class must be added to the editor via [Instance.Editorplugin.AddDebuggerPlugin].
Once added, the [Instance.SetupSession] callback will be called for every [EditorDebuggerSession] available to the plugin, and when new ones are created (the sessions may be inactive during this stage).
You can retrieve the available [EditorDebuggerSession]s via [Instance.GetSessions] or get a specific one via [Instance.GetSession].
...
Note: While the game is running, [Instance.@Globalscope.Print] and similar functions [i]called in the editor[/i] do not print anything, the Output Log prints only game messages.
*/
...
/*
Override this method to enable receiving messages from the debugger. If 'capture' is "my_message" then messages starting with "my_message:" will be passed to the [Instance.Capture] method.
*/
...
/*
Override this method to process incoming messages. The 'session_id' is the ID of the [EditorDebuggerSession] that received the 'message'. Use [Instance.GetSession] to retrieve the session. This method should return true if the message is recognized.
*/
...
Note: Sessions in the array may be inactive, check their state via [Instance.Editordebuggersession.IsActive].
*/
