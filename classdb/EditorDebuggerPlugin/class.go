...
To interact with the debugger, an instance of this class must be added to the editor via [graphics.gd/classdb/EditorPlugin.Instance.AddDebuggerPlugin].

Once added, the [Interface.SetupSession] callback will be called for every [graphics.gd/classdb/EditorDebuggerSession] available to the plugin, and when new ones are created (the sessions may be inactive during this stage).

You can retrieve the available [graphics.gd/classdb/EditorDebuggerSession]s via [Instance.GetSessions] or get a specific one via [Instance.GetSession].
...
To connect on the running game side, use the [graphics.gd/classdb/EngineDebugger] singleton:
...
Note: While the game is running, [graphics.gd/classdb/@GlobalScope.Instance.Print] and similar functions called in the editor do not print anything, the Output Log prints only game messages.
*/
...
type Interface interface {
	// Override this method to be notified whenever a new [graphics.gd/classdb/EditorDebuggerSession] is created. Note that the session may be inactive during this stage.
	SetupSession(session_id int)
	// Override this method to enable receiving messages from the debugger. If 'capture' is "my_message" then messages starting with "my_message:" will be passed to the [Interface.Capture] method.
	HasCapture(capture string) bool
	// Override this method to process incoming messages. The 'session_id' is the ID of the [graphics.gd/classdb/EditorDebuggerSession] that received the 'message'. Use [Instance.GetSession] to retrieve the session. This method should return true if the message is recognized.
...
/*
Returns the [graphics.gd/classdb/EditorDebuggerSession] with the given 'id'.
*/
...
/*
Returns an array of [graphics.gd/classdb/EditorDebuggerSession] currently available to this debugger plugin.
...
func (self *class) SetObject(obj [1]gd.Object) bool {
	if gdextension.Host.Objects.Cast(gdextension.Object(pointers.Get(obj[0])[0]), otype) != 0 {
		self[0] = pointers.AsA[gdclass.EditorDebuggerPlugin](obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.EditorDebuggerPlugin{pointers.Add[gdclass.EditorDebuggerPlugin]([3]uint64{})})
	...
	casted := Instance([1]gdclass.EditorDebuggerPlugin{pointers.New[gdclass.EditorDebuggerPlugin]([3]uint64{uint64(gdextension.Host.Objects.Make(sname))})})
...
func (self class) GetSession(id int64) [1]gdclass.EditorDebuggerSession { //gd:EditorDebuggerPlugin.get_session
	var r_ret = gdextension.Call[gdextension.Object](gd.ObjectChecked(self.AsObject()), methods.get_session, gdextension.SizeObject|(gdextension.SizeInt<<4), &struct{ id int64 }{id})
...
func (self class) GetSessions() Array.Any { //gd:EditorDebuggerPlugin.get_sessions
	var r_ret = gdextension.Call[gdextension.Array](gd.ObjectChecked(self.AsObject()), methods.get_sessions, gdextension.SizeArray, &struct{}{})
	var ret = Array.Through(gd.ArrayProxy[variant.Any]{}, pointers.Pack(pointers.New[gd.Array](r_ret)))
	return ret
}
func (self class) AsEditorDebuggerPlugin() Advanced {
	return Advanced{pointers.AsA[gdclass.EditorDebuggerPlugin](self[0])}
}
func (self Instance) AsEditorDebuggerPlugin() Instance {
	return Instance{pointers.AsA[gdclass.EditorDebuggerPlugin](self[0])}
}
...
func init() {
	gdclass.Register("EditorDebuggerPlugin", func(ptr gd.Object) any { return Instance{pointers.AsA[gdclass.EditorDebuggerPlugin](ptr)} })
}
