...
/*
[graphics.gd/classdb/PolygonOccluder3D] stores a polygon shape that can be used by the engine's occlusion culling system. When an [graphics.gd/classdb/OccluderInstance3D] with a [graphics.gd/classdb/PolygonOccluder3D] is selected in the editor, an editor will appear at the top of the 3D viewport so you can add/remove points. All points must be placed on the same 2D plane, which means it is not possible to create arbitrary 3D shapes with a single [graphics.gd/classdb/PolygonOccluder3D]. To use arbitrary 3D shapes as occluders, use [graphics.gd/classdb/ArrayOccluder3D] or [graphics.gd/classdb/OccluderInstance3D]'s baking feature instead.
