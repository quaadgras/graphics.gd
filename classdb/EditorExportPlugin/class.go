...
/*
[EditorExportPlugin]s are automatically invoked whenever the user exports the project. Their most common use is to determine what files are being included in the exported project. For each plugin, [Instance.ExportBegin] is called at the beginning of the export process and then [Instance.ExportFile] is called for each exported file.
To use [EditorExportPlugin], register it using the [Instance.Editorplugin.AddExportPlugin] method first.
*/
...
/*
Virtual method to be overridden by the user. Called for each exported file before [Instance.CustomizeResource] and [Instance.CustomizeScene]. The arguments can be used to identify the file. 'path' is the path of the file, 'type' is the [Resource] represented by the file (e.g. [PackedScene]), and 'features' is the list of features for the export.
Calling [Instance.Skip] inside this callback will make the file not included in the export.
*/
...
When enabled, [Instance.GetCustomizationConfigurationHash] and [Instance.CustomizeResource] will be called and must be implemented.
*/
...
Implementing this method is required if [Instance.BeginCustomizeResources] returns true.
Note: When customizing any of the following types and returning another resource, the other resource should not be skipped using [Instance.Skip] in [Instance.ExportFile]:
- [AtlasTexture]
- [CompressedCubemap]
- [CompressedCubemapArray]
- [CompressedTexture2D]
...
When enabled, [Instance.GetCustomizationConfigurationHash] and [Instance.CustomizeScene] will be called and must be implemented.
Note: [Instance.CustomizeScene] will only be called for scenes that have been modified since the last export.
*/
...
Implementing this method is required if [Instance.BeginCustomizeScenes] returns true.
*/
...
Return a list of export options that can be configured for this export plugin.
Each element in the return value is a [Dictionary] with the following keys:
- option: A dictionary with the structure documented by [Instance.Object.GetPropertyList], but all keys are optional.
...
*/
func (Instance) _get_export_options(impl func(ptr gdclass.Receiver, platform EditorExportPlatform.Instance) []map[any]any) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args, p_back gdextension.Pointer) {
		var platform = [1]gdclass.EditorExportPlatform{pointers.New[gdclass.EditorExportPlatform]([3]uint64{uint64(gd.UnsafeGet[gdextension.Object](p_args, 0))})}
...
[/codeblock]
*/
func (Instance) _get_export_options_overrides(impl func(ptr gdclass.Receiver, platform EditorExportPlatform.Instance) map[any]any) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args, p_back gdextension.Pointer) {
		var platform = [1]gdclass.EditorExportPlatform{pointers.New[gdclass.EditorExportPlatform]([3]uint64{uint64(gd.UnsafeGet[gdextension.Object](p_args, 0))})}
...
/*
Return true, if the result of [Instance.GetExportOptions] has changed and the export options of preset corresponding to 'platform' should be updated.
*/
...
Note: Use [Instance.GetOption] to check the value of the export options.
*/
...
When called inside [Instance.ExportFile] and 'remap' is true, the current file will not be exported, but instead remapped to this custom file. 'remap' is ignored when called in other places.
'file' will not be imported, so consider using [Instance.CustomizeResource] to remap imported resources.
*/
...
/*
To be called inside [Instance.ExportFile]. Skips the current file, so it's not included in the export.
*/
...
/*
Returns the current value of an export option supplied by [Instance.GetExportOptions].
*/
