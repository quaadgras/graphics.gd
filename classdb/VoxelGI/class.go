...
Note: Meshes should have sufficiently thick walls to avoid light leaks (avoid one-sided walls). For interior levels, enclose your level geometry in a sufficiently large box and bridge the loops to close the mesh. To further prevent light leaks, you can also strategically place temporary [MeshInstance3D] nodes with their [member GeometryInstance3D.gi_mode] set to [Geometryinstance3d.GiModeStatic]. These temporary nodes can then be hidden after baking the [VoxelGI] node.
*/
package VoxelGI
...
Note: [Instance.Bake] works from the editor and in exported projects. This makes it suitable for procedurally generated or user-built levels. Baking a [VoxelGI] node generally takes from 5 to 20 seconds in most scenes. Reducing [member subdiv] can speed up baking.
Note: [GeometryInstance3D]s and [Light3D]s must be fully ready before [Instance.Bake] is called. If you are procedurally creating those and some meshes or lights are missing from your baked [VoxelGI], use call_deferred("bake") instead of calling [Instance.Bake] directly.
*/
