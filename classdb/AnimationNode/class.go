...
/*
Base resource for [graphics.gd/classdb/AnimationTree] nodes. In general, it's not used directly, but you can create custom ones with custom blending formulas.

Inherit this when creating animation nodes mainly for use in [graphics.gd/classdb/AnimationNodeBlendTree], otherwise [graphics.gd/classdb/AnimationRootNode] should be used instead.

You can access the time information as read-only parameter which is processed and stored in the previous frame for all nodes except [graphics.gd/classdb/AnimationNodeOutput].

Note: If multiple inputs exist in the [graphics.gd/classdb/AnimationNode], which time information takes precedence depends on the type of [graphics.gd/classdb/AnimationNode].
...
type Interface interface {
	...
	// When inheriting from [graphics.gd/classdb/AnimationRootNode], implement this virtual method to return whether the 'parameter' is read-only. Parameters are custom local memory used for your animation nodes, given a resource can be reused in multiple trees.
	IsParameterReadOnly(parameter string) bool
	// When inheriting from [graphics.gd/classdb/AnimationRootNode], implement this virtual method to run some code when this animation node is processed. The 'time' parameter is a relative delta, unless 'seek' is true, in which case it is absolute.
	//
	// Here, call the [Instance.BlendInput], [Instance.BlendNode] or [Instance.BlendAnimation] functions. You can also use [Instance.GetParameter] and [Instance.SetParameter] to modify local memory.
	//
	// This function should return the delta.
	Process(time Float.X, seek bool, is_external_seeking bool, test_only bool) Float.X
	// When inheriting from [graphics.gd/classdb/AnimationRootNode], implement this virtual method to override the text caption for this animation node.
	GetCaption() string
	// When inheriting from [graphics.gd/classdb/AnimationRootNode], implement this virtual method to return whether the blend tree editor should display filter editing on this animation node.
	HasFilter() bool
}
...
/*
Adds an input to the animation node. This is only useful for animation nodes created for use in an [graphics.gd/classdb/AnimationNodeBlendTree]. If the addition fails, returns false.
*/
...
/*
Amount of inputs in this animation node, only useful for animation nodes that go into [graphics.gd/classdb/AnimationNodeBlendTree].
*/
...
/*
Returns the object id of the [graphics.gd/classdb/AnimationTree] that owns this node.

Note: This method should only be called from within the [graphics.gd/classdb/AnimationNodeExtension.Instance.ProcessAnimationNode] method, and will return an invalid id otherwise.
*/
...
/*
Blend another animation node (in case this animation node contains child animation nodes). This function is only useful if you inherit from [graphics.gd/classdb/AnimationRootNode] instead, otherwise editors will not display your animation node for addition.
*/
