...
/*
A library of meshes. Contains a list of [graphics.gd/classdb/Mesh] resources, each with a name and ID. Each item can also include collision and navigation shapes. This resource is used in [graphics.gd/classdb/GridMap].
*/
package MeshLibrary
...
You can get an unused ID from [Instance.GetLastUnusedItemId].
*/
...
This name is shown in the editor. It can also be used to look up the item later using [Instance.FindItemByName].
*/
...
/*
When running in the editor, returns a generated item preview (a 3D rendering in isometric perspective). When used in a running project, returns the manually-defined item preview which can be set using [Instance.SetItemPreview]. Returns an empty [graphics.gd/classdb/Texture2D] if no preview was manually set in a running project.
*/
