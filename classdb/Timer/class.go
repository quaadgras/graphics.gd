...
After a timer enters the tree, it can be manually started with [Instance.Start]. A timer node is also started automatically if [member autostart] is true.
...
[/codeblock]
Note: To create a one-shot timer without instantiating a node, use [Instance.Scenetree.CreateTimer].
Note: Timers are affected by [member Engine.time_scale]. The higher the time scale, the sooner timers will end. How often a timer processes may depend on the framerate or [member Engine.physics_ticks_per_second].
*/
package Timer
...
/*
Starts the timer, or resets the timer if it was started already. Fails if the timer is not inside the tree. If 'time_sec' is greater than 0, this value is used for the [member wait_time].
