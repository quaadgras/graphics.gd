...
/*
[graphics.gd/classdb/VisibleOnScreenNotifier2D] represents a rectangular region of 2D space. When any part of this region becomes visible on screen or in a viewport, it will emit a [Instance.OnScreenEntered] signal, and likewise it will emit a [Instance.OnScreenExited] signal when no part of it remains visible.

If you want a node to be enabled automatically when this region is visible on screen, use [graphics.gd/classdb/VisibleOnScreenEnabler2D].

Note: [graphics.gd/classdb/VisibleOnScreenNotifier2D] uses the render culling code to determine whether it's visible on screen, so it won't function unless [graphics.gd/classdb/CanvasItem.Instance.Visible] is set to true.
*/
...
Note: It takes one frame for the [graphics.gd/classdb/VisibleOnScreenNotifier2D]'s visibility to be determined once added to the scene tree, so this method will always return false right after it is instantiated, before the draw pass.
*/
