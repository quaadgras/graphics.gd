...
/*
Returns the editor control responsible for main screen plugins and tools. Use it with plugins that implement [Instance.Editorplugin.HasMainScreen].
Note: This node is a [VBoxContainer], which means that if you add a [Control] child to it, you need to set the child's [member Control.size_flags_vertical] to [Control.SizeExpandFill] to make it use the full available space.
Warning: Removing and freeing this node will render a part of the editor useless and may cause a crash.
*/
...
/*
Returns the specified 3D editor [SubViewport], from 0 to 3. The viewport can be used to access the active editor cameras with [Instance.Viewport.GetCamera3d].
*/
...
/*
Returns true if multiple window support is enabled in the editor. Multiple window support is enabled if [i]all[/i] of these statements are true:
...
/*
Pops up the 'dialog' in the editor UI with [Instance.Window.PopupExclusive]. The dialog must have no current parent, otherwise the method fails.
See also [Instance.Window.SetUnparentWhenInvisible].
*/
...
/*
Pops up the 'dialog' in the editor UI with [Instance.Window.PopupExclusiveCentered]. The dialog must have no current parent, otherwise the method fails.
See also [Instance.Window.SetUnparentWhenInvisible].
*/
...
/*
Pops up the 'dialog' in the editor UI with [Instance.Window.PopupExclusiveCenteredRatio]. The dialog must have no current parent, otherwise the method fails.
See also [Instance.Window.SetUnparentWhenInvisible].
*/
...
/*
Pops up the 'dialog' in the editor UI with [Instance.Window.PopupExclusiveCenteredClamped]. The dialog must have no current parent, otherwise the method fails.
See also [Instance.Window.SetUnparentWhenInvisible].
*/
...
In order to get a reference to the [EditorFeatureProfile], you must load the feature profile using [Instance.Editorfeatureprofile.LoadFromFile].
...
Note: The feature profile that gets activated must be located in the feature_profiles directory, as a file with the .profile extension. If a profile could not be found, an error occurs. The editor configuration folder can be found by using [Instance.Editorpaths.GetConfigDir].
*/
...
/*
Returns the current directory being viewed in the [FileSystemDock]. If a file is selected, its base directory will be returned using [Instance.String.GetBaseDir] instead.
*/
...
/*
Edits the given [Resource]. If the resource is a [Script] you can also edit it with [Instance.EditScript] to specify the line and column position.
*/
