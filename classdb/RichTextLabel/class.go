...
Note: Assignments to [member text] clear the tag stack and reconstruct it from the property's contents. Any edits made to [member text] will erase previous edits made from other manual sources such as [Instance.AppendText] and the push_* / [Instance.Pop] methods.
...
Note: Unlike [Label], [RichTextLabel] doesn't have a [i]property[/i] to horizontally align text to the center. Instead, enable [member bbcode_enabled] and surround the text in a [center] tag as follows: [center]Example[/center]. There is currently no built-in way to vertically align text either, but this can be emulated by relying on anchors/containers and the [member fit_content] property.
*/
...
/*
Adds an [indent] tag to the tag stack. Multiplies 'level' by current [member tab_size] to determine new margin length.
*/
...
/*
Adds [ol] or [ul] tag to the tag stack. Multiplies 'level' by current [member tab_size] to determine new margin length.
*/
...
Note: Using this method, you can't close a tag that was opened in a previous [Instance.AppendText] call. This is done to improve performance, especially when updating large RichTextLabels since rebuilding the whole BBCode every time would be slower. If you absolutely need to close a tag in a future method call, append the [member text] instead of using [Instance.AppendText].
*/
...
Note: If [member threaded] is enabled, this method returns a value for the loaded part of the document. Use [Instance.IsFinished] or [signal finished] to determine whether document is fully loaded.
*/
...
Note: If [member visible_characters_behavior] is set to [Textserver.VcCharsBeforeShaping] only visible wrapped lines are counted.
Note: If [member threaded] is enabled, this method returns a value for the loaded part of the document. Use [Instance.IsFinished] or [signal finished] to determine whether document is fully loaded.
*/
...
/*
Installs a custom effect. This can also be done in the Inspector through the [member custom_effects] property. 'effect' should be a valid [RichTextEffect].
