...
The playback of the animation is controlled by the [member speed_scale] property, as well as each frame's duration (see [Instance.SetFrameDuration]). The animation loops, i.e. it will restart at frame 0 automatically after playing the last frame.
...
You can define any number of textures up to [MaxFrames], but keep in mind that only frames from 0 to [member frames] - 1 will be part of the animation.
*/
...
/*
Sets the duration of any given 'frame'. The final duration is affected by the [member speed_scale]. If set to 0, the frame is skipped during playback.
*/
