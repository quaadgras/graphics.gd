...
Note: Unless otherwise specified, all methods that have angle parameters must have angles specified as [i]radians[/i]. To convert degrees to radians, use [Instance.@Globalscope.DegToRad].
...
This is particularly important for frame-based operations that take place in [Instance.Node.Process], rather than [Instance.Node.PhysicsProcess]. Examples include [Camera3D]s focusing on a node, or finding where to fire lasers from on a frame rather than physics tick.
Note: This function creates an interpolation pump on the [Node3D] the first time it is called, which can respond to physics interpolation resets. If you get problems with "streaking" when initially following a [Node3D], be sure to call [Instance.GetGlobalTransformInterpolated] at least once [i]before[/i] resetting the [Node3D] physics interpolation.
*/
