...
<Aside>
    Don't forget to update the `gd` command! It doesn't stay in sync automatically.
...
build. Explore the [classdb](https://pkg.go.dev/graphics.gd/classdb) to discover all the
functionality that the engine provides. Think about how you can use these features to
build your project.
...
If you stick to using exported struct fields, [startup.Scene](https://pkg.go.dev/graphics.gd/startup#Scene)
and avoid depending on globals or anonymous functions, this may "just work" for you, otherwise, if you are
using [startup.Rendering](https://pkg.go.dev/graphics.gd/startup#Rendering) or require globals or persistent
callables, then there will be internal state not available to the new version of the code. Register
[startup.OnSuspend](https://pkg.go.dev/graphics.gd/startup#OnSuspend) and
[startup.OnRestore](https://pkg.go.dev/graphics.gd/startup#OnRestore) functions to coordinate and transfer
this state.
