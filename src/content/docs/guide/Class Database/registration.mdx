...
<Aside type="caution">
    The class type that you register must always be passed through to the underlying 
    `Extension` or `Singleton` type, so that the correct type is registered with the 
    engine. `graphics.gd` does not support registering inheritance hierarchies.
</Aside>
